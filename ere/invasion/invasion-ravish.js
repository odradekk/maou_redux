 ...
 *     都走不到。**该守卫在 ere 侧不可达**：`monster_data` 对 0–2 列写的
 *     数量（`E:列头+99`）恒 ≥ 1——它算 `second_count` 的六条分支取值都
 *     ≥ 1（ere/dungeon/monster-data.js:437-464），三处提前返回只发生在
 *     `line` 为 3/4/5，而本模块只传 0/1/2。保留只为 1:1，不删不改行为
 *     （issue #486；不可达分支无法用变异守住，故不设变异条目）。

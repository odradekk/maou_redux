 ...
 *     #535 的排版核对（离线夹具 + 源码层）：多列各格是 el-col，引擎把
 *     `config.width` 钳成 24 列网格的跨度（1-24），格的横向位置由跨度
 *     决定、不随前一格文本长度变化，所以编号格写成 `[1] `/`[11] ` 只影响
 *     本格自身填空的长度，不会带着后列左移。**本票未跑 Electron MCP，
 *     实机排版以验收核对为准**（含魔王行与角色行的姓名列是否齐）；

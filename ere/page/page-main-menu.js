 ...
 * 按钮渲染；[100] 与 [109] 已分别随 #44/#117 的真身接通，其余仍占位）——
 * 函数本体的骨架已实现（#23），占位的是这一段；输入分发本体
...
function draw_main_menu() {
  ...
  // :232-319 指令面板其余各项（[101]-[108]、[110]-[888]，可用性依 A/B
  // 计数与 FLAG 状态）：随各自子系统票落地。普查（#129）：这些项的分发
  // 分支全部仍是存根（usershop 的 stub_line_wait），补按钮只会造出「点了
  // 打一行占位」的死入口——按钮与真身同票落地，登记见
  // docs/stub-registry.md 的 DRAW_MAINMENU 行与 @USERSHOP 指令分支表。
  stub_line(
    'DRAW_MAINMENU',

 ...
 *
 * 源: target/ERB/SHOP/SHOP ver1.0.2.ERB  @SELECT_TARGET（:236-330）
 ...
 *   - CLEARLINE 局部重绘与补行排版（:267-277/:306-311）不镜像：ere 控制台
...
/**
 * @IS_ASSISTABLE（SHOP_FUNCTION.ERB:116-133，#FUNCTION）：可当助手返回 0，
 * 否则 1（范围外）/2（CFLAG:x:0 != 2，非助手役）/3（占用中）/4（当前目标）。
 * @param {number} cid 角色 ID
 * @returns {number} 0 = 可当助手
 */
function is_assistable(cid) {
  // :125 范围外（ID 语义改写，见 is_trainable）
  if (cid < 1 || !era.getAddedCharacters().includes(cid)) {
    return 1;
  }
  // :126-127 SIF CFLAG:ARG:0 != 2
  if ((era.get(`cflag:${cid}:0`) || 0) !== 2) {
    return 2;
  }
  // :128-129 SIF CFLAG:ARG:1 != 0
  if ((era.get(`cflag:${cid}:1`) || 0) !== 0) {
    return 3;
  }
  // :130-131 SIF TARGET == ARG（目标不能兼助手）
...
/**
 * @SELECT_TARGET（SHOP ver1.0.2.ERB:236-330）：调教目标选择画面真身。
 *
...
async function select_target() {
  ...
  // :240-251 可调教总数 → 最大页码（0 起；空表为 -1，但首渲染即 RETURN 0，
  // 页码不会被用到）
  const total = era
    .getAddedCharacters()
  ...
  // $INPUT_LOOP（:253-330；LIST_POS/PREV_PAGE 缓存不生效的说明见文件头）
  for (;;) {
    // :266-268 CUSTOMDRAWLINE = / 标题 / DRAWLINE（'=' 线以 isSolid 近似）
    era.drawLine({ isSolid: true });
    era.print('请魔王大人选择将要调教的奴隶人选');
    era.drawLine();
    // :270 CALL SHOW_LIST_TRAINABLE(NO_PAGE,NUM_PAGE,LIST_POS)；:271-273
    // RESULT < 1 → RETURN 0（列表为空 = 取消）
    if (show_list_trainable(no_page, num_page) < 1) {
      return 0;
    }
    // :274-279 补行对齐（CLEARLINE 排版）不镜像，见文件头
    era.drawLine();
    // :280-283 [1000] 上一页 / [999] 返回 / [1002] 其它 / [1001] 下一页
    ...
    if (result === 1002) {
      // :289-291 其它 → CALL MONSTER_PLAY（怪物游玩，存根）；RETURN RESULT
      stub_line('MONSTER_PLAY', '其它（怪物游玩）', '随怪物票');
      return 0;
    }
    if (is_trainable(result) === 0) {
      // :292-296 調教可能な対象 → TARGET = RESULT；FLAG:1 = TARGET；RETURN 1
      era_flag.target = result;
      era.set('flag:1', result);
      return 1;
    }
    if (is_assistable(result) === 0) {
      // :297-301 助手可能な対象 → ASSI = RESULT；FLAG:2 = ASSI；RETURN 1
      era_flag.assi = result;
      era.set('flag:2', result);
      return 1;
    }
    if (result === 1000) {
      // :302-308 上一页（页首不再退；CLEARLINE 不镜像）
      if (no_page > 0) {
        no_page -= 1;
      }
      continue;
    ...
    // :316-318 范围外（RESULT < 0 || RESULT >= CHARANUM）与其余输入：

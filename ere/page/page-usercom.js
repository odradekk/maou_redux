 ...
 *   - :63-68 上次的调教指令：PREVCOM 恒 -1（零指令），不可达；TSTR:90 无
 *     ere 落点，随首条指令票定址；
 *   - :8-13 GETBIT(FLAG:5,34) 的自定义 COM 菜单（CLEAR_TO_POINT +
 *     SHOW_COMMENU）欠账。
 *
...
on('SHOW_USERCOM', async () => {
  // :8-13 GETBIT(FLAG:5,34) → 自定义 COM 菜单（欠账，占位行见下）
  era.println(); // :14 PRINTL（空行）
  era.drawLine(); // :15 DRAWLINE
  // :16 RESETCOLOR —— 无 ere 对应语义，不镜像
  // :17-27 能力表示[100] 污秽表示[101]（交代助手[102]/对换调教[112] 有
  // 守卫）避孕套设定[103]；:28-56 过滤[104-108]；:57-60 调教菜单[990-992]
  ...
  // :61 PRINTC 调教结束[999] —— 本票唯一挂载的菜单项（按钮正文不带 [999]
  // 前缀，引擎自动拼）
  era.printButton('调教结束', 999);
  era.println(); // :62 PRINTL
  // :63-68 PREVCOM > -1 → ＜上次的调教指令：…＞（不可达，见文件头）
});

on('USERCOM', async (result) => {
  // :103 REDRAW 1 —— 不镜像
  // :104-156 分支 100/101/102/112/103/104-108/990/991/992：按钮未挂载，

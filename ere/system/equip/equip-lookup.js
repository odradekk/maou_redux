 ...
 *
 * **跨调用的 W 残留不建模**（记录按次新建）：装饰行与 ELSE 臂不写的
 * 战斗修正列（W:9-W:16）由 #546 起的 equip_st_show 读取——读到的不是
 * 原作的跨调用残留，而是本记录的缺省 undefined（getter 层 `w.列` 恒经
 * 附魔增量 `?? 0` 起算）与查表列，两侧行为一致；但**强度/诅咒（W:2/W:5）
 * 例外**——@EQUIP_DATABASE 对空槽短路时不写这两列，而 EQUIP_SELECT 的
 * 换装守卫会读，原作因此读到上一次调用的旧值。该处按「会话起点 W = 0」
 * 起算（见 ere/system/equip/equip-select.js 的注释）；跨调用的外部残留
 * 仍不建模（#517）。
 */

 ...
 * 由调用方读列——与原作「W 数组当函数间传参与回传载体」同构。
 *
 * **跨调用的 W 残留不建模**（记录按次新建）：本模块对未命中行不写的战斗修正列
 * （W:9-W:16）无人读，故等价（数据文件头注）。但**强度/诅咒（W:2/W:5）例外**
 * ——@EQUIP_DATABASE 对空槽短路时不写这两列，而 EQUIP_SELECT 的换装守卫会读，
 * 原作因此读到上一次调用的旧值。该处按「会话起点 W = 0」起算（见
 * ere/system/equip/equip-select.js 的注释）；跨调用的外部残留仍不建模（#517）。
 */

 ...
 *
 * == 接触面（#214 定死，12 张族票照此写） ==
 *
 *   async com_order(a, s) → { a, s }
 *
 ...
 * @returns {Promise<{a: number, s: number}>} 累加后的 A/S（调用方继续累加）
 */
async function com_order(a = 0, s = 0) {
  ...
  if (abl(10)) {
    plus();
    a += abl(10) * 4;
    era.print(`${name_of('ablname', 10)}LV${abl(10)}(${abl(10) * 4})`);
    s = 1;
  }
  // ABL:21 抖M气质 ×2
  if (abl(21)) {
    plus();
    a += abl(21) * 2;
    era.print(`${name_of('ablname', 21)}LV${abl(21)}(${abl(21) * 2})`);
    s = 1;
  }
  ...
  if (p_talent(122) === 0 && talent(122) === 0) {
    ...
    if (abl(22)) {
      plus();
      a += abl(22) * 3;
      era.print(`${name_of('ablname', 22)}LV${abl(22)}(${abl(22) * 3})`);
      s = 1;
    }
    // ABL:33 百合中毒 ×3
    if (abl(33)) {
      plus();
      a += abl(33) * 3;
      era.print(`${name_of('ablname', 33)}LV${abl(33)}(${abl(33) * 3})`);
      s = 1;
    }
    // TALENT:81 双性恋 +10
    if (talent(81)) {
      plus();
      a += 10;
      era.print(`${name_of('talentname', 81)}(10)`);
      s = 1;
    }
    // TALENT:23 好奇心 +7
    if (talent(23)) {
      plus();
      a += 7;
      era.print(`${name_of('talentname', 23)}(7)`);
      s = 1;
    }
    ...
    if (talent(24)) {
      raw_minus();
      a -= 13;
      era.print(`${name_of('talentname', 24)}(13)`);
      s = 1;
    }
  } else {
    // TALENT:23 好奇心 +5
    if (talent(23)) {
      plus();
      a += 5;
      era.print(`${name_of('talentname', 23)}(5)`);
      s = 1;
    }
    ...
    if (talent(24)) {
      raw_minus();
      a -= 10;
      era.print(`${name_of('talentname', 24)}(10)`);
      s = 1;
    }
  }
  ...
  if (mark(0)) {
    plus();
    a += mark(0) * 5;
    era.print(`${name_of('markname', 0)}LV${mark(0)}(${mark(0) * 5})`);
    s = 1;
  }
  ...
  if (mark(2)) {
    plus();
    a += mark(2) * 3 * t;
    era.print(`${name_of('markname', 2)}LV${mark(2)}(${mark(2) * 3 * t})`);
    s = 1;
  }
  ...
  if (mark(3)) {
    raw_minus();
    a -= mark(3) * 2 * t;
    era.print(`${name_of('markname', 3)}LV${mark(3)}(${mark(3) * 2 * t})`);
    s = 1;
  }
  ...
  if (l_ju) {
    plus();
    a += l_ju * 3;
    era.print(`${name_of('palamname', 4)}LV${l_ju}(${l_ju * 3})`);
    s = 1;
  }
  // PALAM:10 恐怖 L×3
  ...
  if (l_fear) {
    plus();
    a += l_fear * 3;
    era.print(`${name_of('palamname', 10)}LV${l_fear}(${l_fear * 3})`);
    s = 1;
  }
  ...
  for (const [id, contrib] of TARGET_TALENTS) {
    if (talent(id)) {
      ...
      a += contrib;
      era.print(`${name_of('talentname', id)}(${Math.abs(contrib)})`);
      s = 1;
    }
  }
  ...
  for (const [id, contrib] of PLAYER_TALENTS) {
    if (p_talent(id)) {
      plus();
      a += contrib;
      era.print(`${name_of('talentname', id)}(${contrib})`);
      s = 1;
    }
  }

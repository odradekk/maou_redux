 ...
 * 闭环上真实的出口，本壳保留出口、本体（:8-139 的遇敌/日程等）整段欠账，
 * 登记见 docs/stub-registry.md（回合结算票）。
 *
 * 普通档的回合结算本体与 #LATER 空定义尚未接线：接线后本壳与它们在同一条
...
/**
 * 本文件存根化的原作调用名（#PRI 本体的代表调用；完整欠账见
...
on(
  'EVENTTURNEND',
  async () => {
    // :8-139 本体欠账：日程推进（EVENT_NEXTDAY）、随机遇敌（ENTER_ENEMY）、

...
/**
 * @PRITRAIN_MESSAGE 的承载头部（EVENT_BEFORETRAIN.ERB:6-13）。
 *
 ...
 * 消息体（:14-201 的省略设定/初调教/着衣/素质分支叙事）存根化，登记欠账。
 */
async function pritrain_message_head() {
  // :6 调教経験を加算：CFLAG:TARGET:10 += 1
  const target = era_flag.target;
  era.add(`cflag:${target}:10`, 1);

  // :11-13 避免角色错乱的暂存纪录：T:10 = MASTER、T:11 = TARGET、
  ...
  // :15-17 调教テキスト省略設定（FLAG:6 & 1）：省略时的短消息。FLAG:6 未
...
on(
  'EVENTTRAIN',
  async () => {
    // :14-15 主人公の射精を0に（BASE:2 = 射精槽；MASTER 恒角色 0）
    era.set('base:0:2', 0);
    // :16-17 いちおう調教対象と助手も（ASSI >= 0 才写；助手行在下）
    era.set(`base:${era_flag.target}:2`, 0);
    if (era_flag.assi >= 0) {
      era.set(`base:${era_flag.assi}:2`, 0);
    }
    // :19 BASE:TARGET:3 = 0（母乳槽）
    era.set(`base:${era_flag.target}:3`, 0);
    // :20 BASE:MASTER:4 = 0（触手射精槽）
    era.set('base:0:4', 0);

    // :22-25 REPEAT 200：TFLAG:0..199 清 0（引擎建表时已清过一遍静态条目，
    // 原作照写不误——1:1 保留两层写入）
    for (let i = 0; i < 200; i += 1) {
      era.set(`tflag:${i}`, 0);
    }

    // :30-34 調教者は誰か：ASSIPLAY == 0 → PLAYER = MASTER，否则 ASSI
    ...
    // :36-37 记录目标与助手，以备人物切换：ASSI:1 / TARGET:1（flag 槽位）
    era_flag.assi_record = era_flag.assi;
    era_flag.target_record = era_flag.target;

    // :39-43 时常发情ボーナス：TALENT:TARGET:271（时常发情）→ 润滑与欲情
    ...
    // :45 死斗场の収入初期化：TFLAG:402 = 0（200 循环外，独立写入）
    era.set('tflag:402', 0);

    // :48 初始化 TRAIN_NAME（存根：自定义指令名表，静态名已由
    // TrainCommand.yml 承载，见文件头欠账说明）
    stub_line('TRAIN_NAME_INIT', '自定义指令名初始化');

    // :56 CALL PRITRAIN_MESSAGE（承载头部移植 + 消息体存根）

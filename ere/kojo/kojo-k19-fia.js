 ...
 * 计数器。移植不修正，变异条目固定此行为。
 * 原作缺陷 1:1 保留：@MUSEUM_KOUJO_K19 在 :6368-6370 无条件 RETURN，故其后
...
async function kojo_message_com_19(rand) {
  ...
  if (era.get(`tequip:${target}:55`)) {
    // :767
    return colosseum_kojo_19(); // :768-769 // :769-770
  } // :770-771
    ...
    if (chara(target).kojo.交谈 == 0) {
      ...
      if (era.get(`tequip:${target}:53`) == 1) {
        ...
        if (
          ...
          // :4454
          // :4455（同 :4440 那一行的另一条互斥尾段，前缀用同一个变量）
          await era.print(
            player_prefix +
          ...
          // :4457
          // :4458（同 :4440 那一行的另一条互斥尾段，前缀用同一个变量）
          await era.print(
            player_prefix +
              `${target_name}开心的朝着${master_name}撒着娇，说着色色的话语。`,
          ); // :4458
          ...
          // :4460
          // :4461（同 :4440 那一行的另一条互斥尾段，前缀用同一个变量）
          await era.print(
            player_prefix +
              `${target_name}大口大口的喘着气，小小的身体因为快感而像触电一样痉挛个不停。`,
          ); // :4461
...
// #625：死斗场接口 @COLOSSEUM_KOJO_19 的调用点在 KOJO_MESSAGE_COM 的
// `SIF ASSI > 0 && ASSIPLAY → RETURN 0` 之后，运行时不带助手才走到它；助手臂
// 按 1:1 保留，导出只为行为测试能直达这具真身（同 K2/K4/K903 的既有做法）。

...
async function ryouzyoku(arg, rand) {
  ...
  // :58 CALL DUNGEON_RYOUZYOKU（口上前置钩子，按 GET_KOJO_NUM 分发）
  {
    const local = get_kojo_num();
    if ((local >= 100 && local < 140) || local > 1000) {
      await ryouzyoku_kojo_family.call(local - 100, {
        whenMissing: 0,
        args: [],
      });
    }
  ...
  // :168 CALL DUNGEON_RYOUZYOKU_AFTER（口上后置钩子，按 GET_KOJO_NUM 分发）
  {
    const local = get_kojo_num();
    if ((local >= 100 && local < 140) || local > 1000) {
      await ryouzyoku_after_kojo_family.call(local - 100, {
        whenMissing: 0,
        args: [],
      });
    }

 ...
 *
 * == 穿环 CFLAG:348 的跨模块共享标量 P（:5072/:5117/:5162） ==
 *
 ...
 *
 * 守卫（:68-69/:70-71）：FLAG:7 <= 0 跳过、TALENT:169 != 1 跳过；此后按
...
on('EVENTTRAIN', async () => {
  ...
  if (kojo.初调教 == 0) {
    ...
    // :329
    // 崩坏してたら二回目以降へ飛ぶ
    return k9_kojo2(); // :330 CALL k9_kojo2
  } else if (era_flag.assi < 0) {
    // :333
    // 助手の有無をチェック（いない場合は二回目以降へ飛ぶ）
    return k9_kojo2(); // :334 CALL k9_kojo2
  } else if (era.get(`talent:0:122`) == 0) {
    // :343（MASTER 恒角色 0）
    // 你が男じゃなかったら二回目以降
    return k9_kojo2(); // :344 CALL k9_kojo2
  } else if (assi == 20) {
    // :346
    era.drawLine(); // :347
    ...
    return k9_kojo2(); // :581 CALL k9_kojo2
  } // :582
});
 ...
 *
 * 守卫（:799/:801/:805）：FLAG:7 <= 0 跳过、TALENT:169 != 1 跳过、
...
async function kojo_message_com_9(rand) {
  ...
  if (era_flag.selectcom == 2) {
    ...
    if (kojo.肛门爱抚 == 0) {
      ...
      // :1016
      const P = era.get(`palam:${target}:3`) + era.get(`delta:${target}:3`); // :1017
...
async function kojo_message_palamcng_9() {
  ...
  P = era.get(`palam:${target}:3`) + era.get(`delta:${target}:3`); // :6043

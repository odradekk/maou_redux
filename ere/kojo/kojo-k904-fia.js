 ...
 *
 * 源: target/ERB/口上/EVENT_K904_菲娅.ERB（整份 :1-6724）。
 * @EVENTTRAIN / @EVENTEND、@KOJO_MESSAGE_COM_904、参数与刻印变化、
...
on(
  ...
  async () => {
    ...
    // EX_FLAG:104 = 1（变量语义：K904 口上存在标志） // :53
    era_exflag.set(104, 1); // :53
    if (game.kojo.口上开关 == 0) {
      // :55
...
on(
  ...
  async () => {
    ...
    // EX_FLAG:104 = 0（变量语义：K904 口上存在标志） // :59
    era_exflag.set(104, 0); // :59
  },
  TIER.LATER,
);
...
async function kojo_message_com_904(rand) {
  ...
  if (era.get(`tequip:${target}:55`)) {
    // :767
    return colosseum_kojo_904(); // :768-769 // :769-770
  } // :770-771
    ...
    if (chara(target).kojo.交谈 == 0) {
      ...
      if (era.get(`tequip:${target}:53`) == 1) {
        ...
        if (
          ...
          // :4339
          // :4340+:4342+:4343 原作是一整行：无后缀 PRINTFORM 连续不换行，
          ...
          // TFLAG:32 |= 2（变量语义：录像内容位标志） // :4344
          game.kojo.录像内容 |= 2; // :4344
        } else if (era.get(`talent:${target}:76`) == 1) {
          // :4345
          await era.print(
          ...
          // TFLAG:32 |= 2（变量语义：录像内容位标志） // :4351
          game.kojo.录像内容 |= 2; // :4351
        } else if (era.get(`talent:${target}:85`) == 1) {
          // :4352
          await era.print(
          ...
          // TFLAG:32 |= 2（变量语义：录像内容位标志） // :4358
          game.kojo.录像内容 |= 2; // :4358
        } else if (
          ...
          // TFLAG:32 |= 2（变量语义：录像内容位标志） // :4362
          game.kojo.录像内容 |= 2; // :4362
        } else if (
          ...
          // TFLAG:32 |= 2（变量语义：录像内容位标志） // :4365
          game.kojo.录像内容 |= 2; // :4365
        } else {
          // :4366-4367
          await era.printAndWait(
        ...
        if (
          ...
          // :4374
          // :4375+:4377+:4379+:4381 原作是一整行：无后缀 PRINTFORM/PRINT 连续
      ...
      if (era.get(`tequip:${target}:53`) == 1) {
        ...
        if (
          ...
          // TFLAG:32 |= 2（变量语义：录像内容位标志） // :4414
          game.kojo.录像内容 |= 2; // :4414
        } else if (era.get(`talent:${target}:76`) == 1) {
          // :4415
          await era.print(
            `${target_name}稚气的小脸上浮现出和年龄不符的欲望，轻咬着手指对着水晶球作着自我介绍。`,
          ...
          // TFLAG:32 |= 2（变量语义：录像内容位标志） // :4421
          game.kojo.录像内容 |= 2; // :4421
        } else if (era.get(`talent:${target}:85`) == 1) {
          // :4422
          await era.print(
            `${target_name}羞红着脸对着水晶球作着自我介绍，时不时看向${master_name}。`,
          ...
          // TFLAG:32 |= 2（变量语义：录像内容位标志） // :4428
          game.kojo.录像内容 |= 2; // :4428
        } else if (
          era.get(`palam:${target}:5`) >= PALAMLV[4] &&
          (era.get(`talent:${target}:76`) || era.get(`abl:${target}:11`) >= 5)
        ) {
        ...
        // :4439-4440
        // :4440 的 `PRINTFORM %SAVESTR:PLAYER%` 与随后互斥分支的 PRINTFORML
        ...
        if (
          ...
          // :4463-4464
          // :4464（同 :4440 那一行的另一条互斥尾段，前缀用同一个变量）
          await era.print(player_prefix + `${target_name}乖巧的低着头听着。`); // :4464
        } // :4465-4466
        return 0; // :4466-4467
      } // :4467-4468
    } // :4467-4468
...
async function colosseum_kojo_904() {
  ...
  // :6218-:6221）：TALENT:ASSI:121/122 有则「阴茎」/「肉棒」，否则持假阴茎
  ...
  if (era_flag.selectcom == 31) {
    ...
    if (era_flag.assi > 0 && era_flag.assiplay) {
      ...
      // :6160+:6162+:6164+:6165 原作是一整行：无后缀 PRINTFORM/PRINT 连续
  ...
  if (era_flag.selectcom == 21) {
    ...
    if (era_flag.assi > 0 && era_flag.assiplay) {
      ...
      // :6193+:6195+:6197+:6198 同 :6160 组的一整行（#625）
      await era.printAndWait(
  ...
  if (era_flag.selectcom == 27) {
    ...
    if (era_flag.assi > 0 && era_flag.assiplay) {
      ...
      // :6217+:6219+:6221+:6222 同 :6160 组的一整行（#625）
      await era.printAndWait(
        `${assi_name}一边听着悲鸣，一边用` +
          (assi_has_penis ? '肉棒' : assi_has_toy ? '假阴茎' : '') +
...
async function gohoubi_request_koujo_k904() {
  ...
  if (chara(a).stronghold.要求奖赏 == 0) {
    ...
    // :6525+:6527+:6529+:6531+:6533 原作是一整行：:6525 的 PRINTFORM 与
    // :6533 的 PRINTFORMW 参数都是空串，中间是 IF/ELSEIF 三档兽名——合并成
    ...
    const beast_word =
      chara(a).stronghold.要求奖赏 == 1
        ...
        : chara(a).stronghold.要求奖赏 == 2
          ? '豚'
          : '马';
    await era.printAndWait(beast_word); // :6525+:6527+:6529+:6531+:6533

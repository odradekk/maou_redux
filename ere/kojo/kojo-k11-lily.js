    ...
    if (kojo.交谈 === 0) {
      ...
      if (era.get(`tequip:${target}:53`) === 1) {
        ...
        if (
          ...
          // :6841
          // :6842+:6844+:6845 原作是一整行：无后缀 PRINTFORM 不换行，
          // :6844 是 SIF 的插入段，末行 PRINTFORML 才收行（#623）
          const masturbation_talk = chara(target).train.自慰中毒 >= 3; // :6843
          await era.print(
        ...
        if (assi_mao) {
          ...
          if (faced_first) {
            ...
            // :6872
            // :6873+:6875+:6877+:6879 原作是一整行：无后缀 PRINTFORM 链，
            // :6875/:6877 是两个互斥插入段（:6874/:6876 的 IF/ELSEIF，
            // :6877-6878 收支），:6879 的 PRINTFORML 收行（#623）
            const excited =
          ...
          if (faced_first) {
            ...
            // :6900
            // :6901+:6903+:6905+:6907 原作是一整行：无后缀 PRINTFORM 链，
            // :6903/:6905 是两个互斥插入段（:6902/:6904 的 IF/ELSEIF，
            // :6905-6906 收支），:6907 的 PRINTFORML 收行（#623）
            const excited =
              era.get(`tequip:${target}:11`) ||
              era.get(`tequip:${target}:13`) ||
              era.get(`tequip:${target}:14`) ||
      ...
      if (era.get(`tequip:${target}:53`) === 1) {
        ...
        if (
          ...
          // :6928
          // :6929+:6931+:6932 与初回 :6842+:6844+:6845 同型（#623）
          const masturbation_talk = chara(target).train.自慰中毒 >= 3; // :6930
          await era.print(
            `${target_name}介绍了自己的名字和迄今为止的性经验` +
              (masturbation_talk
                ? '、自慰的时候幻想的内容和对象也说出来了'
        ...
        if (assi_mao) {
          ...
          if (faced_first) {
            ...
            // :6959
            // :6960+:6962+:6964+:6966 原作是一整行（与 :6873.. 同型，
          ...
          if (faced_first) {
            ...
            // :6987
            // :6988+:6990+:6992+:6994 原作是一整行（与 :6960.. 同型，
...
async function dog_kojo_11(rand) {
  ...
  if (era_flag.selectcom === 56) {
    ...
    if (kojo.交谈 === 0) {
      ...
      return 0; // :11448-11451
    } // :11448-11452
  } // :11448-11453

  return 0; // :11456-11458
}
...
async function kojo_message_palamcng_11(rand) {
  ...
  if (era_flag.assi > 0 && era_flag.assiplay && era_flag.assi !== 17) {
    // :11465-11466
    return 0; // :11465-11466
  } // :11465-11466

  if (era.get(`tequip:${target}:45`)) {
    // :11468-11469
    return 0; // :11468-11469
  } // :11468-11469

  if (game.train.失神) {
    // :11471-11472
    return 0; // :11471-11472
  } // :11471-11472

  if (era.get(`tequip:${target}:89`)) {
    // :11474-11475
    return 0; // :11474-11475
  } // :11474-11475

  if (era.get(`tequip:${target}:90`)) {
    // :11477-11478
    return 0; // :11477-11478
  } // :11477-11478

  if (era.get(`tequip:${target}:55`)) {
    // :11480
    return 0; // :11480-11481
  } // :11482-11483

  if (era.get(`talent:${target}:9`) === 1) {
    // :11484-11485
    return 0; // :11484-11485
  } // :11484-11485
...
async function kojo_message_markcng_11(rand) {
  ...
  if (era_flag.assi > 0 && era_flag.assiplay && era_flag.assi !== 17) {
    // :11796-11797
    return 0; // :11796-11797
  } // :11796-11797

  if (era.get(`tequip:${target}:45`)) {
    // :11799-11800
    return 0; // :11799-11800
  } // :11799-11800

  if (game.train.失神) {
    // :11802-11803
    return 0; // :11802-11803
  } // :11802-11803

  if (era.get(`tequip:${target}:89`)) {
    // :11805-11806
    return 0; // :11805-11806
  } // :11805-11806

  if (era.get(`tequip:${target}:90`)) {
    // :11808-11809
    return 0; // :11808-11809
  } // :11808-11809

  if (era.get(`talent:${target}:9`) === 1) {
    // :11811-11812
    return 0; // :11811-11812
  } // :11811-11812

  if (era.get(`tequip:${target}:55`)) {
    // :11814-11815
    return 0; // :11814-11815
  } // :11814-11815
...
async function self_kojo_k11(rand) {
  ...
  if (game.train.初吻与自我口上 === 1) {
    ...
    if (era.get(`talent:${target}:9`) === 1) {
      // :11887
      await era.printAndWait(`「嘻嘻嘻……哈哈……嘿嘿……咕嘿嘿${heart(1)}」`); // :11888
      await era.printAndWait(
      ...
      await era.printAndWait(`${target_name}呓语着助手的名字，连续地自慰着……`); // :11893
    } else if (q === 2) {
      // :11895
      await era.printAndWait(
      ...
      if (
        ...
        await era.printAndWait(`「可是…还是不能满足啊啊啊……」`); // :11906
        // CFLAG:261  = 4（变量语义：CFLAG 族，261） // :11907
        kojo.调教后自慰 = 4; // :11907
      } else if (
        era.get(`talent:${target}:85`) &&
        ...
        // :11909
        await era.printAndWait(`「哈啊…啊啊…我到底…在做什么啊…嗯啊啊」`); // :11910
        await era.printAndWait(
        ...
        // CFLAG:261  = 3（变量语义：CFLAG 族，261） // :11914
        kojo.调教后自慰 = 3; // :11914
      } else if (
        ...
        await era.printAndWait(`「呜啊啊…可是…这样摸…好舒服……啊啊…嗯啊啊……」`); // :11919
        // CFLAG:261  = 2（变量语义：CFLAG 族，261） // :11920
        kojo.调教后自慰 = 2; // :11920
        ...
        await era.printAndWait(`「呜呜呜…为什么…我会做这样的事……」`); // :11924
        await era.printAndWait(
          `${target_name}内心充满了罪恶感，却又无法停下自慰的动作………`,
        ); // :11925
        // CFLAG:261  = 1（变量语义：CFLAG 族，261） // :11926
        kojo.调教后自慰 = 1; // :11926
      } // :11926-11927
    } // :11926-11928
  } // :11929-11931
  ...
  if (game.train.初吻与自我口上 === 2) {
    ...
    if (era.get(`talent:${target}:9`) === 1) {
      // :11936
      await era.printAndWait(`「咕嘿……咕嘿嘿嘿……」`); // :11937
      await era.printAndWait(
        `精神已经崩坏的${target_name}凭着本能，沉浸在与${assi_name}的百合之乐中……`,
      ); // :11938
      // CFLAG:262  = 6（变量语义：CFLAG 族，262） // :11939
      kojo.百合PLAY = 6; // :11939
    } else if (
      era.get(`talent:${target}:76`) &&
      ...
      // :11941
      await era.printAndWait(`「嗯啊……啊啊……好，好厉害${heart(1)}」`); // :11942
      await era.printAndWait(
        `${target_name}被${assi_name}用出色的技巧玩弄着身体，连连喘息着，丝毫不掩饰享受的样子。`,
      ); // :11943
      await era.printAndWait(`「继，继续……这样玩我……还想要，更多${heart(1)}」`); // :11944
      await era.printAndWait(
        `${assi_name}面对着${target_name}的反应，笑着继续玩弄着对方身体的敏感点……`,
      ); // :11945
      // CFLAG:262  = 5（变量语义：CFLAG 族，262） // :11946
      kojo.百合PLAY = 5; // :11946
    } else if (
      era.get(`talent:${target}:85`) &&
      ...
      // :11948
      await era.printAndWait(`「嗯啊……啊啊……不，不行啊！」`); // :11949
      await era.printAndWait(
      ...
      // CFLAG:262  = 4（变量语义：CFLAG 族，262） // :11953
      kojo.百合PLAY = 4; // :11953
    } else if (
      ...
      await era.printAndWait(`身体已经完全离不开百合之乐，沉沦其间了……`); // :11958
      // CFLAG:262  = 3（变量语义：CFLAG 族，262） // :11959
      kojo.百合PLAY = 3; // :11959
    } else if (
      ...
      // CFLAG:262  = 2（变量语义：CFLAG 族，262） // :11964
      kojo.百合PLAY = 2; // :11964
    } else if (kojo.百合PLAY < 1 || game.kojo.口上开关 === 2) {
      // :11966
      await era.printAndWait(`「住，住手啊……我们……都是女人啊啊…！」`); // :11967
      await era.printAndWait(
        `${assi_name}把${target_name}按倒在床上，肆意地玩弄着身下的娇躯………`,
      ); // :11968
      // CFLAG:262  = 1（变量语义：CFLAG 族，262） // :11969
      kojo.百合PLAY = 1; // :11969
    } // :11969-11970
  } // :11969-11971
  ...
  if (game.train.初吻与自我口上 === 3) {
    ...
    if (era.get(`talent:${target}:9`) === 1) {
      // :11978
      await era.printAndWait(`「咕呣……咕呣…♪」`); // :11979
      await era.printAndWait(`${target_name}带着茫然的表情舔着阴茎……`); // :11980
      // CFLAG:263  = 4（变量语义：CFLAG 族，263） // :11981
      kojo.朝口交 = 4; // :11981
    } else if (
      era.get(`talent:${target}:76`) === 1 &&
      ...
      // CFLAG:263  = 3（变量语义：CFLAG 族，263） // :11988
      kojo.朝口交 = 3; // :11988
    } else if (
      era.get(`talent:${target}:85`) &&
      (kojo.朝口交 < 3 || game.kojo.口上开关 === 2)
      ...
      // CFLAG:263  = 3（变量语义：CFLAG 族，263） // :11995
      kojo.朝口交 = 3; // :11995
    } else if (
      ...
      // CFLAG:263  = 2（变量语义：CFLAG 族，263） // :12000
      kojo.朝口交 = 2; // :12000
      ...
      // CFLAG:263  = 1（变量语义：CFLAG 族，263） // :12005
      kojo.朝口交 = 1; // :12005
    } // :12005-12006
  } // :12005-12007
  ...
  if (game.train.初吻与自我口上 === 4) {
    ...
    if (era.get(`talent:${target}:9`) === 1) {
      // :12014
      await era.printAndWait(''); // :12015
      // CFLAG:264  = 3（变量语义：CFLAG 族，264） // :12016
      kojo.调教后性交 = 3; // :12016
    } else if (
      ...
      // CFLAG:264  = 2（变量语义：CFLAG 族，264） // :12031
      kojo.调教后性交 = 2; // :12031
      ...
      await era.print(`「呜呜……好舒服……舒服得……已经没办法思考了……」`); // :12036
      await era.printAndWait(
      ...
      // CFLAG:264  = 1（变量语义：CFLAG 族，264） // :12039
      kojo.调教后性交 = 1; // :12039
    } // :12039-12040
  } // :12039-12041
  ...
  if (game.train.初吻与自我口上 === 5) {
    ...
    if (
      ...
      // :12048
      await era.printAndWait(`「哈啊……啊啊……天花板好亮啊……」`); // :12049
      await era.printAndWait(
        `已经精神崩溃的${target_name}被自己的主人抱着，回到了${master_name}的房间………`,
      ); // :12050
      // CFLAG:265  = 2（变量语义：CFLAG 族，265） // :12051
      kojo.夜袭 = 2; // :12051
    } else {
      // :12051-12052
      await era.printAndWait(
      ...
      // CFLAG:265  = 1（变量语义：CFLAG 族，265） // :12061
      kojo.夜袭 = 1; // :12061
    } // :12061-12062
  } // :12061-12063
  ...
  if (game.train.初吻与自我口上 === 6) {
    ...
    if (era.get(`talent:${target}:85`) && era.get(`mark:${target}:3`) < 3) {
      // :12070
      await era.printAndWait(''); // :12071
    } else if (era.get(`mark:${target}:3`) === 3) {
      // :12073
      await era.printAndWait(''); // :12074
    } else if (era.get(`talent:${target}:76`)) {
      // :12076
      await era.printAndWait(''); // :12077
    } else {
      // :12078-12079
      await era.printAndWait(''); // :12080
    } // :12080-12081
  } // :12080-12082
  ...
  if (game.train.初吻与自我口上 === 11) {
    ...
    if (kojo.妊娠发觉 === 0) {
      ...
      if (era.get(`talent:${target}:9`) === 1) {
        ...
        await era.printAndWait(`${target_name}的精神彻底崩溃了……`); // :12094
      } else if (
        era.get(`talent:${target}:85`) &&
      ...
      // CFLAG:271  = 1（变量语义：CFLAG 族，271） // :12120
      kojo.妊娠发觉 = 1; // :12120
    } else {
      // :12120-12121
      ...
      if (era.get(`talent:${target}:9`) === 1) {
        ...
        await era.printAndWait(`${target_name}流着口水，目瞪口呆的样子…`); // :12125
      } else if (
        era.get(`talent:${target}:85`) &&
        chara(target).event.妊娠相手 === 1
      ) {
      ...
      // CFLAG:271  = 1（变量语义：CFLAG 族，271） // :12151
      kojo.妊娠发觉 = 1; // :12151
    } // :12151-12152
  } // :12151-12153
  ...
  if (game.train.初吻与自我口上 === 12) {
    ...
    if (kojo.生产 === 0) {
      ...
      if (era.get(`talent:${target}:9`) === 1) {
        // :12162
        await era.printAndWait(`「哈……哈啊……啊啊啊！啊啊啊啊！」`); // :12163
        await era.printAndWait(
        ...
        await era.printAndWait(`${target_name}抱着你的孩子，幸福地笑了……`); // :12168
      } else {
        // :12169-12170
        await era.printAndWait(`「呼，呼……孩，孩子出生了……」`); // :12171
      } // :12171-12172
      // CFLAG:272  = 1（变量语义：CFLAG 族，272） // :12173
      kojo.生产 = 1; // :12173
    } else {
      // :12173-12174
      ...
      if (era.get(`talent:${target}:9`) === 1) {
        ...
        // :12180
        await era.printAndWait(`「看，这是我们又一个孩子呢……嘻嘻」`); // :12181
        await era.printAndWait(`${target_name}抱着你的孩子，幸福地笑了……`); // :12182
      } else {
        // :12183-12184
        await era.printAndWait(`「呼，呼……孩，孩子出生了……」`); // :12185
      } // :12185-12186
      // CFLAG:272  = 1（变量语义：CFLAG 族，272） // :12187
      kojo.生产 = 1; // :12187
    } // :12187-12188
  } // :12187-12189
  ...
  if (game.train.初吻与自我口上 === 13) {
    ...
    if (era.get(`talent:${target}:85`) || era.get(`talent:${target}:76`)) {
      ...
      if (era.get(`talent:${target}:153`)) {
        ...
        // :12202
        await era.printAndWait(`「看，魔王大人来探望你了，快点打个招呼吧？」`); // :12203
        await era.printAndWait(`${target_name}哄着孩子……`); // :12204
      } // :12204-12205
    } // :12206-12207
    // CFLAG:273  = 1（变量语义：CFLAG 族，273） // :12207
    kojo.育儿室 = 1; // :12207
  } // :12207-12208

  if (game.train.初吻与自我口上 === 14) {
    // :12213
    await era.printAndWait(`「啊啊、那孩子走了………」`); // :12214
    // CFLAG:274  = 1（变量语义：CFLAG 族，274） // :12215
    kojo.亲离 = 1; // :12215
  } // :12215-12216
  ...
  if (game.train.初吻与自我口上 === 999) {
    ...
    if (era.get(`talent:${target}:85`)) {
      // :12223
      await era.printAndWait(''); // :12224
    } else {
      // :12225-12226
      await era.printAndWait(''); // :12227
    } // :12227-12228
  } // :12227-12229

  // TFLAG:13  = 0（变量语义：TFLAG 族，13） // :12234
  game.train.初吻与自我口上 = 0; // :12234

  return 0; // :12234-12236
}
...
async function colosseum_kojo_11(rand) {
  ...
  if (era_flag.selectcom === 27) {
    ...
    return 0; // :12579-12581
  } // :12582-12584

  if (era_flag.selectcom === 51) {
    // :12587
    await era.printAndWait(''); // :12588
    return 0; // :12588-12589
  } // :12588-12590

  return 0; // :12593-12596
}
...
async function ntr_koujo_k11(_rand, p = 0) {
  ...
  if (p === 1) {
    if (era0(`talent:${target}:76`) || era0(`talent:${target}:85`)) {
      ...
      // :12609+:12611+:12613+:12615 原作是一整行：无后缀 PRINTFORM 链，
      // :12611/:12613 是互斥插入段（:12610 的 IF），:12615 的 PRINTFORMW 收行（#623）
      await era.printAndWait(
      ...
      // :12622+:12624+:12626+:12628 与 :12609.. 同型（#623）
      await era.printAndWait(
        `双手被抓住的${target_name}拼命挣扎着，但狂王只是哈哈大笑着用` +
          (king_has_penis ? `双腿之间的巨根` : `粗大的假阳具`) +
    ...
    if (era0(`talent:${target}:76`) || era0(`talent:${target}:85`)) {
      ...
      // :12638+:12640+:12642+:12644 与 :12609.. 同型（#623）
      await era.printAndWait(
        `${target_name}被` +
      ...
      // :12655+:12657+:12659+:12661 与 :12609.. 同型（#623）
      await era.printAndWait(
        `${target_name}的肛门被` +
          (king_has_penis ? `狂王的巨根` : `粗大的假阳具`) +
          `撑开了肛门，插了进去。`,
    ...
    if (era0(`talent:${target}:76`) || era0(`talent:${target}:85`)) {
      ...
      // :12683+:12685+:12687 原作是一整行：两互斥插入段 + PRINTFORMW 收行（#623）
      await era.printAndWait(
        (king_has_penis ? `狂王的巨根` : `粗大的假阳具`) +
      ...
      // :12700+:12702+:12704 与 :12683.. 同型（#623）
      await era.printAndWait(
        (king_has_penis ? `狂王的巨根` : `粗大的假阳具`) +
    ...
    if (era0(`talent:${target}:76`) || era0(`talent:${target}:85`)) {
      ...
      if (king_has_penis) {
        await era.print(`被不同的阴茎持续侵犯着，精液一次次注入又漏出…`); // :12724
      } else {
        await era.print(`被假阳具持续地侵犯着，爱液不住地渗出……`); // :12726
      }
    }
...
async function gohoubi_request_koujo_k11() {
  ...
  if (request === 0) {
    ...
    await era.printAndWait(`${name}想要和魔王交媾`); // :12929
  } else if (request === 6) {
    await era.printAndWait(`${name}想要品尝精液`); // :12932
  } else if (request === 7) {
    await era.printAndWait(`${name}想要进行一次乱交`); // :12935
  } else if (request === 8) {
    await era.printAndWait(`${name}想要品尝尿液`); // :12938
  } else if (request === 9) {
    await era.printAndWait(`${name}想要和童贞的魔族少年交媾`); // :12941
  }
}
...
function aegi_k11(args, arg = 0, rand) {
  ...
  return result; // :13466-13468 RETURNF LOCALS
}

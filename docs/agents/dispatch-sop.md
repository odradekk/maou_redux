...
6. 涉及界面输出的工单，运行对应的输出比对样本 `node tools/compare/cli.js --sample <名>`；涉及大量 `:N` 引用的工单，运行 `node tools/trace-check.mjs --anchor-quality --all`。弱锚点数量只能减少，修正后同步降低 `ANCHOR_QUALITY_BASELINE`，不能提高基线或往 `ANCHOR_QUALITY_BY_FILE` 加冻结项。
7. 对照附录的验收参考核对新增校验机制、`must_mention`、引擎行为验证、原作引用、存档兼容等项。
...
8. **检查原作引用和有意保留的行为。** 文件头指向 `target/` 中实际存在的文件和函数；原作缺陷登记到 #14，尽可能用反向变异验证「按原作保留」（#116，M214/M218）。
...
12. **差异归因只能解释已批准的行为差异。** `tools/compare/rules.js` 可以记录引擎交互形式或 #60 简体转换造成的差异，不能隐藏移植缺陷（#338 曾把缺少 `$` 归为显示差异）。覆盖整段行号的规则须单独检查：该段参与比对的条目确实都属于相应未实现功能，已实现输出必须排除并有测试确认其缺失会进入 `unexplained`。

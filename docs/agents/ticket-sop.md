...
**第四种代价：把「消费另一张在跑的票的输出面」的票派出去，两边都要返工。** 阶段 4 里 #211 第三段的活是给对拍锁基线四数，而 #212 正在改的恰好是 ere 侧输出——#212 一合并，#211b 锁的 `train-natural 537/176/2549` 立刻变成 `552/176/2536`，两条基线锁当场红。数字往好的方向动（匹配涨、存根降、unexplained 恒 0），但**返工是免不了的**，而且返工的人是派单方而非交付方。

**判据比第三种更简单：这张票的验收标准里有没有「某个数字」，而那个数字由另一张在跑的票决定。** 有就等，没有就派。基线四数、`LEDGER_COUNT_BASELINE`、跳过数、端到端天数都属这一类。

**第三种代价：两张并行票被分别要求「给同一个范围外函数建存根」，结果建出两份。** #177（H8）与 #178（H9）都调 `@SELL_EX_ITEM`（EX 道具票的范围，两票都够不着），简报各自写了「建域内存根 + 登记」——合并后 `dungeon-room.js` 有一个无参版、`dungeon-town.js` 有一个带 cid 版，登记表两行。两个 agent 都照简报做了，错在派单：写 #178 的简报时 #177 正在建同一个。

**派单前扫一遍新票会调到的范围外函数，与在跑的票比对**（票面的「范围」表 + `grep -oP "CALL \K[A-Z_0-9]+" <源 ERB> | sort -u`，减去票面自己的函数名与 `ere/` 里已有的）。撞上了就在简报里指名「那个存根由 #N 建，等它合并后复用，别另起」。

漏掉也不致命——**两份存根都在登记表里，没有东西是隐形的**，接真身的票 grep 得到两行。验收时把两行改名互相指认（本轮做法：「房间侧调用点」「城镇侧调用点」，各自注明另一份在哪），比强行合并成一份、为一个存根拉出跨模块依赖要好。
...
做法：**给每张在跑的票留一段并写下来**，下一张从上一段的末尾之后起。一轮四张并行的分配长这样：

```
#182 → M500 起   #185 → M520 起   #176 → M540 起   #180 → M560 起
```

**号段宽度要按票的规模给，不能一律 20。** 阶段 3 收口时又撞了一次：#177（H8 房间设施，14 函数 8 种设施）实际写了 22 条 M600–M621，而下一段起点是 #179 的 M620——溢出两号，验收时只能把 M620/M621 改派到空段 M633/M634。**估法看票面的验收清单**：「每个 X 各有一条测试」这类逐项条目，条数至少等于 X 的个数，再加接线、原作缺陷反向钉、观测锚点自证。**十函数以上的票直接留 40**，号段浪费不要紧（M 号只是引用锚点，不连续无害），撞号要在合并时手工解 `tools/mutations/*.mjs` 的冲突，贵得多。

...
**注释**：只写「为什么这样写」，用当前代码自身能解释的说法；不写来源标注（外部文件路径、函数名、行号），不写「同某处一致」这类脱离当前代码的说明。

**缺陷**：按普通产品问题处理。玩家看得到的功能、数值、文本错误直接修；修法取决于玩家看到的提示时，以提示为准。断言旧行为的测试改成断言修好后的行为，并补变异条目守住；测试标题、断言说明或注释改了，同步改变异条目的 `must_mention`、`test_name`。修复改变存档中的状态时，按 ADR-0006 判断是否抬存档版本。修缺陷的工单在完成评论里列出每个缺陷位置和处理方式（改正、删除或只改注释）。

// 引擎契约欠账台账（issue #91）：契约测试（test/engine-contract.test.js）查出
...
// 不用行号（#72 教训：行号随编辑腐烂），注释挪动只要片段仍在就不算发霉。
// 两道门在 tools/engine-contract-check.mjs：
//   只能变短：新条目必须同时显式改检查器内嵌的 #91 基线（LEDGER_BASELINE），
//     在版本库差异里看得见——台账是「尚未对齐」的欠账表，不是「可以不
//     一致」的许可；
//   不许发霉：witness 必须仍能在 test/helpers/era-fixture.js 里找到——分歧
//     被修复（镜像补上、注释删除）后条目必须同步删，留着就红。
// 消化一条欠账 = 把镜像补进夹具（第一层范围）或立「永久偏离」用例，然后
...
export const ENGINE_CONTRACT_LEDGER = [
  {
    ...
    desc: 'printAndWait 的内部等待不镜像：引擎 = print + waitAnyKey 两步组合（app.asar 逐字），夹具只 print——waits / inputs_consumed 只记显式 waitAnyKey，缝对「等待」的观测口径由 fixture 用例钉住',
  ...
  },
  {
    ...
    desc: "waitAnyKey 的等待不取输入：引擎内部走 input({any:true}) 真等一回渲染层回包，夹具只留痕（inputs_consumed 记 {api:'waitAnyKey'}），不动预置输入队列",

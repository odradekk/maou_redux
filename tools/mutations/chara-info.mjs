...
export default [
  ...
  {
    desc: 'M12357 名册 [1600] 漏写 await（重叠检测直接报错：统一卖春积极性的等待与名册下一轮输入并发，#557/#545）',
    file: 'ere/page/page-chara-info.js',
    find: `      await uniform_bitch_level();
      continue;`,
    replace: `      uniform_bitch_level(); // 变异：漏 await
      continue;`,
    tests: ['page-chara-info'],
    must_mention: '疑似漏写 await',
  },
  {
    desc: 'M12358 名册 [1700] 漏写 await（重叠检测直接报错：换号页的等待与名册下一轮输入并发，#557/#545）',
    file: 'ere/page/page-chara-info.js',
    find: `      await chara_number_swap();
      continue;`,
    replace: `      chara_number_swap(); // 变异：漏 await
      continue;`,
    tests: ['page-chara-info'],
    must_mention: '疑似漏写 await',
  },

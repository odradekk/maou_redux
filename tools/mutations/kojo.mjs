...
export default [
  ...
  {
    desc: 'M1770 K4 SELF_KOJO 成熟出售真身调用丢失（#338）',
    file: 'ere/kojo/kojo-k4-stoic.js',
    find: '      await sell_maturo_k0(target); // CALL SELL_MATURO_K0（源行 4305）',
  ...
  {
    desc: 'M1689 K1 成熟出售真身调用丢失（#338）',
    file: 'ere/kojo/kojo-k1-confident.js',
    find: '      await sell_maturo_k0(target, { rand }); // CALL SELL_MATURO_K0 // :7002',
    replace: '      // 变异：成熟出售真身调用丢失',
    tests: ['sell-maturo'],
    must_mention: '14 个活动口上的成熟出售事件均进入市场真身',
  },
  ...
  {
    desc: 'M1728 K3 成熟出售真身调用丢失（#338）',
    file: 'ere/kojo/kojo-k3-noble.js',
    find: '      await sell_maturo_k0(target, { rand }); // CALL SELL_MATURO_K0 // :7672',
    replace: '      // 变异：成熟出售真身调用丢失',
    tests: ['sell-maturo'],
    must_mention: '14 个活动口上的成熟出售事件均进入市场真身',
  },
...
const { arena_slave_point, com_after_arena } = require('#/system/train/com-colosseum'); // 变异：顶层 require（#288 守卫的靶子）`,
  ...
  },
  {
    ...
    file: 'ere/kojo/kojo-k7-heart.js',
    find: `      const assi_dildo_8257 = !assi_has_penis_8255 && era0('item:4') == 1;`,
    replace: `      const assi_dildo_8257 = !assi_has_penis_8255 && era0('item:PBAND') == 1; // 变异：回退字符串具名寻址`,
    tests: ['kojo-k7-heart'],
  ...
  },
  {
    ...
    file: 'ere/kojo/kojo-k9-diamond.js',
    find: '        era.get(`item:${PBAND}`) == 1; // :7088',
    replace:
  ...
  ...[
    [4702, '调教后自慰', '11926', 'TFLAG:13=1 推进 CFLAG:261'],
    [4703, '百合PLAY', '11969', 'TFLAG:13=2 推进 CFLAG:262'],
    [4704, '朝口交', '12005', 'TFLAG:13=3 推进 CFLAG:263'],
    [4705, '调教后性交', '12039', 'TFLAG:13=4 推进 CFLAG:264'],
    [4706, '夜袭', '12061', 'TFLAG:13=5 推进 CFLAG:265'],
    [4707, '妊娠发觉', '12120', 'TFLAG:13=11 推进 CFLAG:271'],
    [4708, '生产', '12173', 'TFLAG:13=12 推进 CFLAG:272'],
    [4709, '育儿室', '12207', 'TFLAG:13=13 推进 CFLAG:273'],
    [4710, '亲离', '12215', 'TFLAG:13=14 推进 CFLAG:274'],
  ].map(([id, state, ref, must_mention]) => ({
    desc: `M${id} SELF_KOJO_K11 ${state}首次状态写错（#242）`,
    file: 'ere/kojo/kojo-k11-lily.js',
    find: `kojo.${state} = 1; // :${ref}`,
    replace: `kojo.${state} = 99; // :${ref}`,
    tests: ['kojo-k11-lily'],
  ...
  {
    desc: 'M4711 SELF_KOJO_K11 夜袭 F=1 分支错位（#242）',
    file: 'ere/kojo/kojo-k11-lily.js',
    find: '      if (f === 1) {\n        // :12056',
    replace: '      if (f === 2) {\n        // :12056 变异',
    tests: ['kojo-k11-lily'],
  ...
  },
  {
    ...
    replace: '      if (s >= 4) {\n        // :12030 变异',
    tests: ['kojo-k11-lily'],
    must_mention: '调教后性交读取 aftertrain 的 S',
  },
  {
    desc: 'M4713 SELF_KOJO_K11 结束不清 TFLAG:13（#242）',
    file: 'ere/kojo/kojo-k11-lily.js',
    find: '  game.train.初吻与自我口上 = 0; // :12234',
  ...
  },
  {
    ...
    replace:
      "    const animal = ['', '猪', '狗', '马'][request]; // :12917 变异",
    tests: ['kojo-k11-lily'],
    must_mention: 'GOHOUBI_REQUEST：十种要求',
  },
  {
  ...
  {
    desc: 'M4687 PALAMCNG_11 润滑首超严格大于改为大于等于（#242）',
    file: 'ere/kojo/kojo-k11-lily.js',
    find: '  if (p1 > PALAMLV[2] && kojo.首次润滑Lv2 === 0) {',
    replace: '  if (p1 >= PALAMLV[2] && kojo.首次润滑Lv2 === 0) {',
    tests: ['kojo-k11-lily'],
    must_mention: 'PALAM+UP 恰等于 Lv2 时不触发首超',
  },
  ...[
    [4688, '首次润滑Lv2', '11522', '四类参数首超由 PALAM+UP 严格越过 Lv2'],
    [4689, '首次欲情Lv2', '11560', '四类参数首超由 PALAM+UP 严格越过 Lv2'],
    [4690, '首次耻情Lv2', '11579', '四类参数首超由 PALAM+UP 严格越过 Lv2'],
    [4691, '首次恐怖Lv2', '11598', '四类参数首超由 PALAM+UP 严格越过 Lv2'],
    [4692, '首次C绝顶', '11618', '四类首次绝顶分别推进 CFLAG:225-228'],
    [4693, '首次V绝顶', '11645', '四类首次绝顶分别推进 CFLAG:225-228'],
    [4694, '首次A绝顶', '11691', '四类首次绝顶分别推进 CFLAG:225-228'],
    [4695, '首次B绝顶', '11743', '四类首次绝顶分别推进 CFLAG:225-228'],
    [4696, '处女丧失', '11786', '处女丧失累计 UP11+UP12'],
  ].map(([id, state, ref, must_mention]) => ({
    desc: `M${id} PALAMCNG_11 ${state}状态写错（#242）`,
    file: 'ere/kojo/kojo-k11-lily.js',
    find: `kojo.${state} = 1; // :${ref}`,
    replace: `kojo.${state} = 99; // :${ref}`,
    tests: ['kojo-k11-lily'],
  ...
  },
  {
    ...
    tests: ['kojo-k8-spade'],
    must_mention: '非扶她尾调 SELL_MATURO_K0',
  },
  {
  ...
  {
    desc: 'M3335 K13 SELF_KOJO 成熟出售真身调用丢失（#338）',
    file: 'ere/kojo/kojo-k13-protector.js',
    find: `      await sell_maturo_k0(target, { rand }); // CALL SELL_MATURO_K0 // :4880`,
    replace: `      // 变异：成熟出售真身调用丢失`,
    tests: ['sell-maturo'],
    must_mention: '14 个活动口上的成熟出售事件均进入市场真身',
  },
  ...
  {
    desc: 'M3586 K15 SELF_KOJO 成熟出售真身调用丢失（#338）',
    file: 'ere/kojo/kojo-k15-clever.js',
    find: `      await sell_maturo_k0(target, { rand }); // CALL SELL_MATURO_K0 // :5317`,
    replace: `      void 0; // 变异：删除成熟出售真身调用`,
    tests: ['sell-maturo'],
    must_mention: '14 个活动口上的成熟出售事件均进入市场真身',
  },
  {
...
const gohoubi_request_koujo_family = new DispatchFamily(
  ...
  },
  {
    ...
    file: 'ere/kojo/kojo-forced-payment.js',
    find: '        cost = play * 100 + rand_n(1000) + 1000; // :34',
    replace: '        cost = play * 10 + rand_n(1000) + 1000; // :34',
    tests: ['kojo-forced-payment'],
    must_mention: '档债务',
  },
  {
    desc: 'M11409 强制肉偿的债务抵完清零值改错（= 0 改 = 1，欠金永远回不到 0，#544）',
    file: 'ere/kojo/kojo-forced-payment.js',
    find: '    chara(arg).patch.借款 = 0; // :74',
    replace: '    chara(arg).patch.借款 = 1; // :74',
    tests: ['kojo-forced-payment'],
    must_mention: '债务被抵完时清零',
  },
  {
    desc: 'M11410 强制肉偿的债务累加方向改反（+= COST 改 -= COST，欠债越还越多，#544）',
    file: 'ere/kojo/kojo-forced-payment.js',
    find: '    chara(arg).patch.借款 += cost; // :76',
    replace: '    chara(arg).patch.借款 -= cost; // :76',
    tests: ['kojo-forced-payment'],
    must_mention: '-19000',
  },
  {
    desc: 'M11411 强制肉偿的拍片判定取反（!RAND:3 改 RAND:3，2/3 概率拍片，#544）',
    file: 'ere/kojo/kojo-forced-payment.js',
    find: '  if (!rand_n(3)) {',
    replace: '  if (rand_n(3)) {',
    tests: ['kojo-forced-payment'],
  ...
  },
  {
    ...
    file: 'ere/kojo/kojo-forced-payment.js',
    find: '  if (era.get(`talent:${arg}:122`)) {',
    replace: '  if (era.get(`talent:${arg}:121`)) {',
    tests: ['kojo-forced-payment'],
    must_mention: 'ANAL 档经验名（肛门）',
  },
  {
    desc: 'M11418 强制肉偿的善恶值截断方向改错（Math.trunc 改 Math.floor，-10/4 从 -2 变 -3，#544）',
    file: 'ere/kojo/kojo-forced-payment.js',
    find: '  const local = Math.trunc((-1 * play) / 4);',
    replace: '  const local = Math.floor((-1 * play) / 4);',
    tests: ['kojo-forced-payment'],
    must_mention: 'LOCAL 显示与截断方向',
  },
  {
    desc: 'M11419 强制肉偿的开场行插值顺序错位（「由于{名}欠的」写成「{名}由于欠的」，显示名跑到句首，#544）',
    file: 'ere/kojo/kojo-forced-payment.js',
  ...
  {
    desc: 'M11420 强制肉偿的触发门槛取反（heroine_bitch 的 !RAND:3 改 RAND:3，2/3 概率被强偿，#544）',
    file: 'ere/kojo/kojo-dungeon-bitch.js',
    find: '    !era.get(`talent:${arg}:0`) &&\n    !rand_n(3)\n  ) {\n    // :77 CALL 强制肉偿(ARG)',
    replace:
  ...
  },
  {
    ...
    file: 'ere/kojo/kojo-k10-club.js',
    find: `      // :6797+:6799+:6801+:6802 与 :6763+:6765+:6767+:6768 同型（#622）
      const assi_has_penis =
      ...
      const assi_has_strap =
        ...
        era.get(\`talent:\${assi}:122\`) != 1 &&
        era.get('item:4') == 1;`,
    replace: `      // :6797+:6799+:6801+:6802 与 :6763+:6765+:6767+:6768 同型（#622）
      const assi_has_penis =
        era.get(\`talent:\${assi}:121\`) == 1 ||
        era.get(\`talent:\${assi}:122\`) == 1;
      const assi_has_strap =
  ...
  },
  {
    ...
    find:
      ...
      '      ); // :7495+:7497',
    replace:
      '      await era.print(`「和${benki_player_name()}`); // :7495（变异：拆回）\n' +
  ...
  // 该拆的行没被并」——两侧各钉几条。
  {
    ...
    find:
      ...
      '      ); // :7495+:7497',
    replace:
      '      await era.printAndWait(`「和来同时用小穴和菊花来做爱了♪」`); // :7495+:7497（变异：丢名字）',
    tests: ['kojo-k0-tender'],
    must_mention:
  ...
  },
  {
    ...
    find:
      ...
      '      ); // :7516+:7518',
    replace:
      '      await era.printAndWait(`「和用小穴做爱做到潮如泉涌咯♪」`); // :7516+:7518（变异：丢名字）',
    tests: ['kojo-k0-tender'],
    must_mention:
      '#599 BENKI_KOUJO：FLAG:62 = 3/4/5/6 的首句在名字位置插 FLAG:64 的对象名',
  },
  {
    ...
    find:
      ...
      '      ); // :7537+:7539',
    replace:
      '      await era.printAndWait(`「和用菊花做爱做到湿滑不已咯♪」`); // :7537+:7539（变异：丢名字）',
    tests: ['kojo-k0-tender'],
    must_mention:
      '#599 BENKI_KOUJO：FLAG:62 = 3/4/5/6 的首句在名字位置插 FLAG:64 的对象名',
  },
  {
    ...
    find:
      ...
      '      ); // :7558+:7560',
    replace:
      '      await era.printAndWait(`「给予先生的肉棒大人的『施舍』哦♪」`); // :7558+:7560（变异：丢名字）',
    tests: ['kojo-k0-tender'],
    must_mention:
      '#599 BENKI_KOUJO：FLAG:62 = 3/4/5/6 的首句在名字位置插 FLAG:64 的对象名',
  },
  ...
  // （每处合并后的整行断言见对应测试文件；条目按工单给的 M12140 起编号）
  {
    ...
    find:
      ...
      '            ); // :5233+:5234',
    replace:
      '            await era.print(`「啊啊…如果和你舌吻的话…会变得很舒服吧…？」`); // 变异：拆回\n' +
      '            await era.print(`${target_name}为了展示环而伸出了舌头………`); // 变异：拆回',
    tests: ['kojo-k8-spade'],
  ...
  },
  {
    ...
    find:
      ...
      '            ); // :5237+:5238',
    replace:
      '            await era.print(`「啊啊、总觉环好奇怪…必须要和你接吻来确认状况呢」`); // 变异：拆回\n' +
      '            await era.print(`${target_name}一边害羞的笑着，一边闭上眼撅起了嘴………`); // 变异：拆回',
    tests: ['kojo-k8-spade'],
  ...
  },
  {
    ...
    file: 'ere/kojo/kojo-k11-lily.js',
    find: '    await era.printAndWait(`${name}提出了和` + animal + `进行兽交的请求`); // :12915+:12917+:12919+:12921+:12923',
    replace:
      '    await era.print(`${name}提出了和`); // 变异：拆回\n' +
      '    await era.print(animal); // 变异：拆回\n' +
      '    await era.printAndWait(`进行兽交的请求`); // 变异：拆回',
    tests: ['kojo-k11-lily'],
  ...
  },
  {
    ...
    replace:
      ...
      "          `${'\\u3000'}只要是魔王大人的命令来的话、${sc()}一定会在这里…用这个`,\n" +
      '        ); // 变异：拆回\n' +
      '        if (devoted) era.print(`魔王大人专用`); // 变异：拆回\n' +
      '        if (lewd) era.print(`淫乱`); // 变异：拆回\n' +
      '        if (female_dog) era.print(`牝犬`); // 变异：拆回\n' +
      '        if (greedy) era.print(`贪欲`); // 变异：拆回\n' +
      '        if (virgin) era.print(`处女`); // 变异：拆回\n' +
      '        era.print(`小穴来、给今天看到的大家侍奉也说不定呢。`); // 变异：拆回',
    tests: ['kojo-k3-noble'],
  ...
  },
  {
    ...
    replace:
      ...
      "    await era.print('【最喜欢阴茎】'); // 变异：拆回\n" +
      "    if (cold) await era.print('【性冷淡便器】'); // 变异：拆回\n" +
      "    if (modest) await era.print('【看似忠贞的便器出道】'); // 变异：拆回\n" +
      "    if (wet) await era.print('【又粘又湿】'); // 变异：拆回\n" +
      "    if (pleased) await era.print('【愉悦的脸】'); // 变异：拆回\n" +
      "    if (has_penis) await era.print('【有鸡鸡的奴隶】'); // 变异：拆回\n" +
      '    if (rand_n(3) === 0) {\n' +
      "      await era.print('【操我】'); // 变异：拆回\n" +
      '    } else if (rand_n(2) === 0) {\n' +
      "      await era.print('【肛门免费】'); // 变异：拆回\n" +
      '    } else {\n' +
      "      await era.print('【母猪】'); // 变异：拆回\n" +
      '    }\n' +
      '    await era.printAndWait(\n' +
      "      '之类的话。络绎不绝的魔族男人，将嘴巴、肛门等等地方都侵犯了，精液流得到处都是。',\n" +
      '    ); // 变异：拆回',
  ...
  },
  {
    ...
    find:
      ...
      '      (in_dungeon',
    replace:
      "  await era.print(`${fs_bitch('LOOKS', arg)}`); // 变异：拆回\n" +
      '  await era.printAndWait(\n' +
      "    '' +\n" +
      '      (in_dungeon',
    tests: ['kojo-dungeon-bitch-log'],
  ...
  },
  {
    ...
    file: 'ere/kojo/kojo-dungeon-bitch.js',
    find: '  era.print(`[${num}] 卖春积极性 - ${level}`); // :1157+:1160+:1162+:1164+:1167',
    replace:
  ...
  },
  {
    ...
    replace:
      ...
      '            if (excited) {\n' +
      "              await era.print('却乐在其中'); // 变异：拆回\n" +
      '            } else if (painful) {\n' +
      "              await era.print('无比痛苦'); // 变异：拆回\n" +
      '            }\n' +
      '            await era.print(`地回应着。`); // 变异：拆回',
    tests: ['kojo-k11-lily'],
  ...
  },
  {
    ...
    find:
      ...
      '      ); // :12609+:12611+:12613+:12615',
    replace:
      '      await era.print(`双手被抓住的${target_name}拼命挣扎着，但狂王只是哈哈大笑着用`); // 变异：拆回\n' +
      '      if (king_has_penis) {\n' +
      '        await era.print(`双腿之间的巨根`); // 变异：拆回\n' +
      '      } else {\n' +
      '        await era.print(`粗大的假阳具`); // 变异：拆回\n' +
      '      }\n' +
      '      await era.printAndWait(`径直插入了${target_name}的处女蜜穴之中。`); // 变异：拆回',
    tests: ['kojo-k11-lily'],
  ...
  },
  {
    ...
    replace:
      ...
      '        await era.print(`粗大的假阳具`); // 变异：拆回\n' +
      '      }\n' +
      '      await era.printAndWait(`径直插入了${target_name}的蜜穴之中。`); // 变异：拆回',
    tests: ['kojo-k11-lily'],
    must_mention: '#623 NTR P=1',
  },
  {
    ...
    find:
      ...
      '      ); // :12638+:12640+:12642+:12644',
    replace:
      '      await era.print(`${target_name}被`); // 变异：拆回\n' +
      '      if (king_has_penis) {\n' +
  ...
  },
  {
    ...
    find:
      ...
      '          (king_has_penis ? `狂王的巨根` : `粗大的假阳具`) +\n' +
      '          `撑开了肛门，插了进去。`,\n' +
      '      ); // :12655+:12657+:12659+:12661',
    replace:
      '      await era.print(`${target_name}的肛门被`); // 变异：拆回\n' +
      '      if (king_has_penis) {\n' +
      '        await era.print(`狂王的巨根`); // 变异：拆回\n' +
      '      } else {\n' +
      '        await era.print(`粗大的假阳具`); // 变异：拆回\n' +
      '      }\n' +
      '      await era.printAndWait(`撑开了肛门，插了进去。`); // 变异：拆回',
    tests: ['kojo-k11-lily'],
    must_mention: '#623 NTR P=2',
  },
  {
  ...
  },
  {
    ...
    replace:
      '          await era.print(line_head); // 变异：拆回\n' +
      '          await era.print(`搭话后、${target_name}晃动着腰部继续说着充满爱意的话语`); // 变异：拆回',
    tests: ['kojo-k9-diamond'],
  ...
  },
  {
    ...
    find:
      ...
      '      ); // :7085+:7087+:7089+:7090',
    replace:
      '      await era.print(`${assi_name}看着${target_name}舔着`); // 变异：拆回\n' +
      '      if (assi_penis) {\n' +
      '        await era.print(`阴茎`); // 变异：拆回\n' +
      '      } else if (assi_band) {\n' +
      '        await era.print(`假阴茎`); // 变异：拆回\n' +
      '      }\n' +
      '      await era.printAndWait(`露出了十分愉悦的表情……`); // 变异：拆回',
    tests: ['kojo-k9-diamond'],
  ...
  },
  {
    ...
    find:
      ...
      '      ); // :7119+:7121+:7123+:7124',
    replace:
      '      await era.print(`${assi_name}听着${target_name}的悲鸣继续用`); // 变异：拆回\n' +
      '      if (assi_penis) {\n' +
      '        await era.print(`阴茎`); // 变异：拆回\n' +
      '      } else if (assi_band) {\n' +
      '        await era.print(`假阴茎`); // 变异：拆回\n' +
      '      }\n' +
      '      await era.printAndWait(`来毫不留情地蹂蹑的${target_name}的小穴。`); // 变异：拆回',
    tests: ['kojo-k9-diamond'],
    must_mention: '#623 死斗场',
  },
  {
    ...
    find:
      ...
      '      ); // :7144+:7146+:7148+:7149',
    replace:
      '      await era.print(`${assi_name}一边听着${target_name}的悲鸣一边用`); // 变异：拆回\n' +
      '      if (assi_penis) {\n' +
      '        await era.print(`阴茎`); // 变异：拆回\n' +
      '      } else if (assi_band) {\n' +
      '        await era.print(`假阴茎`); // 变异：拆回\n' +
      '      }\n' +
      '      await era.printAndWait(`毫不留情地继续蹂蹑${target_name}的屁眼。`); // 变异：拆回',
    tests: ['kojo-k9-diamond'],
    must_mention: '#623 死斗场',
  },
  {
    ...
    find:
      ...
      '        beast +\n' +
      '        `交尾试一试呢~、能不能事先帮我准备好呢~？」`,\n' +
      '    ); // :7414+:7416+:7418+:7420+:7422',
    replace:
      '    await era.print(`「呐~魔王大人、我想和`); // 变异：拆回\n' +
      '    await era.print(beast); // 变异：拆回\n' +
      '    await era.printAndWait(`交尾试一试呢~、能不能事先帮我准备好呢~？」`); // 变异：拆回',
    tests: ['kojo-k9-diamond'],
  ...
  },
  {
    ...
    file: 'ere/kojo/kojo-k3-noble.js',
    find: '            await era.printAndWait(line_head + `着……竟然…」`); // :4777',
    replace:
      '            await era.printAndWait(line_head); // 变异：拆回\n' +
  ...
  },
  {
    ...
    replace:
      ...
      '        await era.print(rand_n(2) === 0 ? `一脸陶醉的表情` : `专心地`); // 变异：拆回\n' +
      '        await era.printAndWait(`和野狗用舌头缠绕在一起了。`); // 变异：拆回',
    tests: ['kojo-k3-noble'],
  ...
  },
  {
    ...
    find:
      ...
      '        ); // :7383+:7385+:7387+:7389',
    replace:
      '        await era.print(`「啊啊~…我的`); // 变异：拆回\n' +
      '        await era.print(both_holes ? `两个小穴` : `小穴还有屁股`); // 变异：拆回\n' +
      '        await era.printAndWait(`都要生出来、要生出来了啊~……啊啊~！！」`); // 变异：拆回',
    tests: ['kojo-k3-noble'],
  ...
  },
  {
    ...
    find:
      ...
      '      ); // :8492+:8494+:8496+:8497',
    replace:
      '      await era.print(`${assi_name}用`); // 变异：拆回\n' +
      '      if (assi_penis) {\n' +
      '        await era.print(`大鸡巴`); // 变异：拆回\n' +
      '      } else if (assi_band) {\n' +
      '        await era.print(`假阳具`); // 变异：拆回\n' +
      '      }\n' +
      '      await era.printAndWait(`让${target_name}吸着，露出了愉悦的表情……`); // 变异：拆回',
    tests: ['kojo-k3-noble'],
  ...
  },
  {
    ...
    replace:
      ...
      '        await era.print(`假阳具`); // 变异：拆回\n' +
      '      }\n' +
      '      await era.printAndWait(`将${target_name}的小穴毫不留情地侵犯着……`); // 变异：拆回',
    tests: ['kojo-k3-noble'],
    must_mention: '#623 死斗场·助手器具名整行',
  },
  {
    ...
    replace:
      ...
      '        await era.print(`假阳具`); // 变异：拆回\n' +
      '      }\n' +
      '      await era.printAndWait(`将${target_name}的肛穴毫不留情地侵犯着……`); // 变异：拆回',
    tests: ['kojo-k3-noble'],
    must_mention: '#623 死斗场·助手器具名整行',
  },
  {
    ...
    replace:
      '    await era.print(`「${self_call(a)}…这场战斗完后想要跟…`); // 变异：拆回\n' +
      '    await era.print(beast); // 变异：拆回\n' +
      '    await era.printAndWait(`交配想得受不了了~…！」`); // 变异：拆回',
    tests: ['kojo-k3-noble'],
    must_mention: '#623 奖赏请求·野兽名整行',
  },
  ...
  },
  {
    ...
    file: 'ere/kojo/kojo-k3-noble.js',
    find: '            await era.printAndWait(line_head + `着……唔！」`); // :4779',
    replace:
      '            await era.printAndWait(line_head); // 变异：拆回\n' +
      '            await era.printAndWait(`着……唔！」`); // 变异：拆回',
    tests: ['kojo-k3-noble'],
    must_mention: '#623 强制排泄·两穴与屈辱支的整行',
  },
  {
    ...
    replace:
      '              await era.printAndWait(line_head); // 变异：拆回\n' +
      "              await era.printAndWait(`请、请怜悯下…！${'\\u3000'}啊？${'\\u3000'}啊啊、不要啊啊……」`); // 变异：拆回",
    tests: ['kojo-k3-noble'],
    must_mention: '#623 强制排泄·两穴与屈辱支的整行',
  },
  {
    ...
    file: 'ere/kojo/kojo-k3-noble.js',
    find: "            const line_head = `「原、原谅我…啊啊啊啊！！${'\\u3000'}`; // :4810",
    replace:
      '            const line_head = `「原、原谅我…啊啊啊啊！！`; // 变异：丢行尾全角空格',
    tests: ['kojo-k3-noble'],
    must_mention: '含前缀行尾全角空格',
  },

  // —— #623 第 2 轮验收返工：K11 四族的第五支（`:6885`/`:6913`/`:6972`/`:7000`）与
  // 第六支（`:6888`/`:6916`/`:6975`/`:7003`）原先没有断言，去掉 `line_head + `
  ...
  },
  {
    ...
    find:
      ...
      '            ); // :6913',
    replace:
      '            await era.print(`的语言挑逗、${target_name}结结巴巴地回答着`); // 变异：丢前缀',
    tests: ['kojo-k11-lily'],
    must_mention: '#623 COM56 初回通常（',
  },
  {
    ...
    find:
      ...
      '            ); // :6916',
    replace:
      '            await era.print(`的语言挑逗、${target_name}听清楚了吗…`); // 变异：丢前缀',
    tests: ['kojo-k11-lily'],
    must_mention: '#623 COM56 初回通常（',
  },
  {
  ...
  },
  {
    ...
    file: 'ere/kojo/kojo-dungeon-ravish.js',
    find: '      await era.print(`脸上流露着沉浸在了羞耻与情欲之中的神色……`); // :330+:335',
    replace:
  ...
  },
  {
    ...
    find:
      ...
      '    ); // :504+:511',
    replace:
      '    await era.print(`${name_of(arg)}从事了`); // 变异：拆回\n' +
  ...
  },
  {
    ...
    file: 'ere/kojo/kojo-dungeon-ravish.js',
    find: "        await era.print(face_front + '的神情为屈服的喜悦与口水所浸染……'); // :339",
    replace:
      '        await era.print(face_front); // 变异：拆回\n' +
      "        await era.print('的神情为屈服的喜悦与口水所浸染……'); // 变异：拆回",
    tests: ['kojo-dungeon-ravish'],
  ...
  },
  {
    ...
    find:
      ...
      "            : '可以奖励我与马') +\n" +
      "        '做爱吗…？」',\n" +
      '    ); // :8133+:8135+:8137+:8139+:8141',
    replace:
      '    era.print(`「要是${sc()}打倒勇者的话…`); // 变异：拆回\n' +
      '    era.printAndWait(\n' +
      '        (request_kind === 1\n' +
      "          ? '可以奖励我与犬'\n" +
      '          : request_kind === 2\n' +
  ...
  },
  {
    ...
    file: 'ere/kojo/kojo-k0-tender.js',
    find: '          await era.printAndWait(front_4475 + dump_mid + `请您好好地观赏……」`); // :4484',
    replace:
  ...
  },
  {
    ...
    file: 'ere/kojo/kojo-k0-tender.js',
    find: '          await era.printAndWait(front_4475 + dump_mid + `请您好好地疼爱……」`); // :4486',
    replace:
      '          era.print(front_4475 + dump_mid); // 变异：拆回\n' +
  ...
  },
  {
    ...
    find:
      ...
      '      ); // :7815+:7817+:7819+:7820',
    replace:
      "          (assi_has_cock ? '用假阳具' : assi_with_band ? '用肉棒' : '') +\n" +
      '          `毫不留情地持续蹂躙着${target_name}的阴道……`,\n' +
      '      ); // :7815+:7817+:7819+:7820',
    tests: ['kojo-k0-tender'],
    must_mention: '21：肉棒词',
  },
  {
    ...
    file: 'ere/kojo/kojo-dungeon-ravish.js',
    find: "          await era.printAndWait(back_2674 + '将点燃的蜡烛倾倒了上去'); // :2679",
    replace:
  ...
  },
  {
    ...
    find:
      ...
      '          ); // :4156+:4158+:4159',
    replace:
      '          await era.print(`${target_name}将自己的本名、至今为止的性经验`); // 变异：拆回\n' +
      '          if (masturbation) {\n' +
      '            await era.print(`甚至自慰时想的什么`); // 变异：拆回\n' +
      '          }\n' +
  ...
  },
  {
    ...
    find:
      ...
      '          ); // :4172+:4174',
    replace:
      '          await era.print(`在和${player_name}`); // 变异：拆回\n' +
      '          await era.print(\n' +
  ...
  },
  {
    ...
    replace:
      '          await era.print(`${target_name}将自己的本名、至今为止的性经验`); // 变异：拆回\n' +
      '          if (masturbation) {\n' +
      '            await era.print(`甚至手淫时想到的什么`); // 变异：拆回\n' +
      '          }\n' +
      '          await era.print(`都微笑地讲了出来……`); // 变异：拆回',
    tests: ['kojo-k10-club'],
  ...
  },
  {
    ...
    find:
      ...
      '          ); // :4223+:4225',
    replace:
      '          await era.print(`在和${master_name}`); // 变异：拆回\n' +
      '          await era.print(\n' +
      '            `会话的过程中，${target_name}扭动着腰呢喃着充满爱意的话语。`,\n' +
      '          ); // 变异：拆回',
    tests: ['kojo-k10-club'],
  ...
  },
  {
    ...
    find:
      ...
      '      ); // :6763+:6765+:6767+:6768',
    replace:
      '      await era.print(`${assi_name}`); // 变异：拆回\n' +
      '      if (assi_has_penis) {\n' +
      '        await era.print(`坚硬的雄性器`); // 变异：拆回\n' +
      '      }\n' +
      '      if (assi_has_strap) {\n' +
      '        await era.print(`粗大的假阳具`); // 变异：拆回\n' +
      '      }\n' +
  ...
  },
  {
    ...
    find:
      ...
      '      ); // :6797+:6799+:6801+:6802',
    replace:
      '      await era.print(`${assi_name}听着${target_name}的悲鸣`); // 变异：拆回\n' +
      '      if (assi_has_penis) {\n' +
      '        await era.print(`用坚硬的雄性器`); // 变异：拆回\n' +
      '      }\n' +
      '      if (assi_has_strap) {\n' +
      '        await era.print(`用粗大的假阳具`); // 变异：拆回\n' +
      '      }\n' +
      '      await era.printAndWait(`${target_name}的肛门被无慈悲的继续蹂躏着。`); // 变异：拆回',
    tests: ['kojo-k10-club'],
    must_mention: '同一行里拼「假阳具」',
  },
  {
  ...
  },
  {
    ...
    file: 'ere/kojo/kojo-k6-wicked.js',
    find: '          await era.print(`戴上口塞的${target_name}左右摇着头………`); // :3971+:3973\n',
    replace:
  ...
  },
  {
    ...
    file: 'ere/kojo/kojo-k6-wicked.js',
    find: '          await era.print(`配合地戴上口塞的${target_name}粗重急促地喘息着………`); // :3985+:3987\n',
    replace:
      '          await era.print(`配合地戴上口塞的${target_name}`); // 变异：拆回\n' +
  ...
  },
  {
    ...
    file: 'ere/kojo/kojo-k6-wicked.js',
    find: '          await era.print(`配合地戴上口塞的${target_name}带着期待地晃动着………`); // :4000+:4002\n',
    replace:
      '          await era.print(`配合地戴上口塞的${target_name}`); // 变异：拆回\n' +
      '          await era.print(`带着期待地晃动着………`); // 变异：拆回\n',
    tests: ['kojo-k6-wicked'],
  ...
  },
  {
    ...
    file: 'ere/kojo/kojo-k6-wicked.js',
    find: '          await era.print(`${target_name}习以为常地被口塞塞住嘴眼色朦胧………`); // :4025+:4027\n',
    replace:
  ...
  },
  {
    ...
    file: 'ere/kojo/kojo-k6-wicked.js',
    find: '          await era.print(`戴上口塞的${target_name}左右摇着头………`); // :4035+:4037\n',
    replace:
      '          await era.print(`戴上口塞的${target_name}`); // 变异：拆回\n' +
      '          await era.print(`左右摇着头………`); // 变异：拆回\n',
    tests: ['kojo-k6-wicked'],
  ...
  },
  {
    ...
    find:
      ...
      '          ); // :4342+:4344\n',
    replace:
      '          await era.print(`在和${player_name}会话的过程中，`); // 变异：拆回\n' +
  ...
  },
  {
    ...
    find:
      ...
      '        ); // :6516+:6518\n',
    replace:
      '        await era.printAndWait(`${assi_name}苦笑着和${target_name}`); // 变异：拆回\n' +
  ...
  },
  {
    ...
    find:
      ...
      '        ); // :6526+:6528\n',
    replace:
      '        await era.printAndWait(`尝到百合滋味的${target_name}嬉笑着`); // 变异：拆回\n' +
  ...
  },
  {
    ...
    find:
      ...
      '          ); // :4311+:4313+:4314\n',
    replace:
      '          await era.print(`面带微笑的${target_name}介绍了自己的本名和性经验`); // 变异：拆回\n' +
      "          await era.print((dirty_exp ? `，甚至还有手淫的时候想到的内容` : '') +\n" +
      '              `……`); // 变异：拆回\n',
    tests: ['kojo-k6-wicked'],
    must_mention:
  ...
  },
  {
    ...
    find:
      ...
      '          ); // :4383+:4385+:4386\n',
    replace:
      '          await era.print(`${target_name}面带微笑地介绍了自己的本名和性经验`); // 变异：拆回\n' +
      "          await era.print((dirty_exp_4384 ? `，甚至还有手淫的时候想到的内容` : '') +\n" +
      '              `……`); // 变异：拆回\n',
    tests: ['kojo-k6-wicked'],
    must_mention:
      '#621 录像自我介绍（SIF ABL:31）：前缀与收行段合成一条输出（:4311/:4383 组）',
  },
  {
    ...
    find:
      '      await era.printAndWait(\n' +
      '        `${assi_name}将` +\n' +
      "          (assi_has_penis_7493 ? `阴茎` : '') + // :7493\n" +
      "          (assi_dildo_7495 ? `假阳具` : '') + // :7495\n" +
      '          `塞入${target_name}的口中。她吞吐着，脸上带有几分愉悦的表情……`,\n' +
      '      ); // :7491+:7493+:7495+:7496\n',
    replace:
      '      await era.printAndWait(`${assi_name}将`); // 变异：拆回\n' +
      "      await era.printAndWait((assi_has_penis_7493 ? `阴茎` : '') + // :7493\n" +
      "          (assi_dildo_7495 ? `假阳具` : '') + // :7495\n" +
      '          `塞入${target_name}的口中。她吞吐着，脸上带有几分愉悦的表情……`); // 变异：拆回\n',
    tests: ['kojo-k6-wicked'],
    must_mention:
  ...
  },
  {
    ...
    find:
      '      await era.printAndWait(\n' +
      '        `${assi_name}边听着惨叫边用` +\n' +
      "          (assi_has_penis_7526 ? `阴茎` : '') + // :7526\n" +
      "          (assi_dildo_7528 ? `假阳具` : '') + // :7528\n" +
      '          `毫不留情地蹂躏着${target_name}的阴道……`,\n' +
      '      ); // :7524+:7526+:7528+:7529\n',
    replace:
      '      await era.printAndWait(`${assi_name}边听着惨叫边用`); // 变异：拆回\n' +
      "      await era.printAndWait((assi_has_penis_7526 ? `阴茎` : '') + // :7526\n" +
      "          (assi_dildo_7528 ? `假阳具` : '') + // :7528\n" +
      '          `毫不留情地蹂躏着${target_name}的阴道……`); // 变异：拆回\n',
    tests: ['kojo-k6-wicked'],
    must_mention:
      'COLOSSEUM_KOJO_6：SC31/21/27 助手无 121/122 且持假阳具（item:4）→ 整行含「假阳具」',
  },
  {
    ...
    find:
      '      await era.printAndWait(\n' +
      '        `${assi_name}边听着惨叫边用` +\n' +
      "          (assi_has_penis_7550 ? `阴茎` : '') + // :7550\n" +
      "          (assi_dildo_7552 ? `假阳具` : '') + // :7552\n" +
      '          `蹂躏着${target_name}那鲜嫩的肛门……`,\n' +
      '      ); // :7548+:7550+:7552+:7553\n',
    replace:
      '      await era.printAndWait(`${assi_name}边听着惨叫边用`); // 变异：拆回\n' +
      "      await era.printAndWait((assi_has_penis_7550 ? `阴茎` : '') + // :7550\n" +
      "          (assi_dildo_7552 ? `假阳具` : '') + // :7552\n" +
      '          `蹂躏着${target_name}那鲜嫩的肛门……`); // 变异：拆回\n',
    tests: ['kojo-k6-wicked'],
    must_mention:
      'COLOSSEUM_KOJO_6：SC31/21/27 助手无 121/122 且持假阳具（item:4）→ 整行含「假阳具」',
  },
  {
    ...
    find:
      ...
      '          ); // :4348+:4350+:4352+:4354\n',
    replace:
      '          await era.print(talk_front_4342); // 变异：拆回\n' +
      '          await era.print(`会话的过程中，${target_name}` +\n' +
      '              (excited_4349\n' +
      '                ? `带着快乐的语调`\n' +
      '                : painful_4351\n' +
      '                  ? `带着痛苦的语调`\n' +
  ...
  },
  {
    ...
    find:
      ...
      '          ); // :4420+:4422+:4424+:4426\n',
    replace:
      '          await era.print(talk_front_4414); // 变异：拆回\n' +
      '          await era.print(`会话的过程中，${target_name}` +\n' +
      '              (excited_4421\n' +
      '                ? `带着快乐的语调`\n' +
      '                : painful_4423\n' +
  ...
  },
  {
    ...
    file: 'ere/kojo/kojo-k6-wicked.js',
    find: '    await era.printAndWait(request_animal_7812 + `性交啦♪」`); // :7806+:7808+:7810+:7812\n',
    replace:
  ...
  },
  {
    ...
    replace:
      ...
      '        await era.print(`阴茎搅动着、精液不停的溢了出来………`); // 变异：拆回\n' +
      '      } else {\n' +

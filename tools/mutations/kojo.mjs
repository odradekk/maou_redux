...
const gohoubi_request_koujo_family = new DispatchFamily(
  ...
  {
    desc: 'M12731 女版卖春·客行（:205+:212）拆回两条（#624：名字与客合计又各占一行）',
    file: 'ere/kojo/kojo-dungeon-bitch.js',
    find: "        era.print(`${name_of(arg)}` + (girl[0] && man[0] ? `${locals}、` : '')); // :205+:212",
    replace:
      '        era.print(`${name_of(arg)}`); // 变异：拆回\n' +
      '        await era.print(\n' +
      "          girl[0] && man[0] ? `${locals}、` : '',\n" +
      '        ); // 变异：拆回',
    tests: ['kojo-dungeon-bitch'],
    must_mention: ':205+:212 与 :213+:217 各占一条',
  },
  {
    desc: 'M12732 女版卖春·客行（:213+:217）拆回两条（#624）',
    file: 'ere/kojo/kojo-dungeon-bitch.js',
    find:
      '        await era.print(\n' +
      ...
      '        ); // :213+:217',
    replace:
      "        await era.print(girl[0] && man[0] ? '于是' : ''); // 变异：拆回\n" +
      '        await era.print(`以${locals}为对手`); // 变异：拆回',
    tests: ['kojo-dungeon-bitch'],
    must_mention: ':205+:212 与 :213+:217 各占一条',
  },
  {
    desc: 'M12733 女版卖春·街中客行（:233+:237）拆回两条（#624）',
    file: 'ere/kojo/kojo-dungeon-bitch.js',
    find:
      '          await era.print(\n' +
      "            (girl[0] && man[0] ? '于是' : '') + `以${locals}为对手`,\n" +

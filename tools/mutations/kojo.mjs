...
const gohoubi_request_koujo_family = new DispatchFamily(
  ...
  {
    desc: 'M12750 K0 灌肠肛塞脱着·「主人…那」（:4475..:4484）拆回多条（#624）',
    file: 'ere/kojo/kojo-k0-tender.js',
    find:
      '          await era.printAndWait(\n' +
      '            `「主人…${sc()}那` +\n' +
      ...
      "                  : '出来的地方也') +\n" +
      '              `请您好好地观赏……」`,\n' +
      '          ); // :4475+:4477+:4479+:4481+:4484',
    replace:
      '          era.print(`「主人…${sc()}那`); // 变异：拆回\n' +
      "          era.print(dump_tier === 0 ? '排泄的地方也' : dump_tier === 1 ? '肮脏的地方也' : '出来的地方也'); // 变异：拆回\n" +
      '          await era.printAndWait(`请您好好地观赏……」`); // 变异：拆回',
    tests: ['kojo-k0-tender'],
    must_mention: ':4475+:4477+:4484 合成一条',
  },
  {

...
// #612 起 +17（M12296-M12315：按钮正文补回原作的「- 」分隔符——补回点被改回时
// 对应的 rendered 断言必须红；返工轮 M12307-M12315 补的是数据表驱动的六个调用点、
// 苏生名单，以及反向那条——部位菜单误加「- 」也必须红）
...
export default [
  ...
  // —— #612 返工轮：数据表驱动的调用点（前缀拼在调用点，表里的 label 另有文本行用途）——
  {

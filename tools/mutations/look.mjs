...
export default [
  ...
  // 能出文本的 LOVE 下标只有 28 个（分支表去掉恒不显示的 50），满旋钮角色里
  // 「分数 > 3 的下标」最多 23 个（含那个 50），rank 23 以后全是分数 ≤ 3 的下标、
  // 必被门槛 continue —— 上界取 [23, 100] 里任何值都逐字同输出（把 25…32 逐个
  // 代入排序 + 显示，22 项与顺序完全一致，见 #591 完成报告）。于是本票改钉三处
  // **能观测**的：上界压进可观测区（22）、显示门槛（4）、心形上限（5），三条都由
  // test/look.test.js 的「满旋钮角色的显示面上限」拦下。
  make(
    11960,

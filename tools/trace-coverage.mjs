 ...
 */
export const PENDING_BASELINE = 1; // #543（S2）合并态实测（并上含 #542/#544 的 master b021867）：
// 本票把 魔改新增/處刑改寫.ERB 做进 ere/event/event-execution-batch.js。合并态
// 10（#541）− 7（#542 把 DEBUG小白娘／MOD 五个文件／img.ERB 判死）− 1（#544
// 强制肉偿）− 1（本票处刑改写）= 1，与 `node tools/trace-check.mjs --coverage`
// 的重测一致（待移植 1 = 统一卖春积极性.ERB，S4 的移植目标，该票交付时再 −1）。
// 数字取自重测，既不相加也不取一侧；历次说明保留在下面，不改写。
// #542 合并态实测 2（并上 #544 的 master f45e31f）：RULINGS 表加
// DEBUG小白娘2024ver0.0.14.ERB、MOD/ 五个文件、魔改新增/img.ERB 七条判死
// （依据 #540 范围决定 2–4），七份文件从「待移植」直接进「已判定不实现」；
// master 侧 #544 已把 魔改新增/强制肉偿.ERB 减 1。

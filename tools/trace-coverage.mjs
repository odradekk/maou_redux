...
//
// #460（Q3）单独的说明：工单范围内的 COMF0_愛撫.ERB／COMF50_ローション.ERB／
// COMF63_貝あわせ.ERB 核实后确认功能代码早已完整实现（非本票新写），此前已
// 不在待移植的分子里，本票未改动其判定；203/205 的功能代码同样早已完整
// （com-colosseum.js 的 MONSTER_CONFIGS 与 202/204/206 同构共享 monster_com），
// 本票只补齐了追溯锚点证据（tools/trace-refs/com-colosseum.mjs）使
// trace-coverage 的判定从「待移植」翻正为「已移植」——五个文件只减 2，不是 5。
//

 ...
 */
// 合并态实测（#545 并上含 #544 的 master f45e31f）：已移植 310／部分移植 8／
// 已判定不实现 10／待移植 8／纯声明 10。两票各降 1 且起点相同（#545 的 10 → 9、
// #544 的 10 → 9），合并后是 8 而不是任一单票的 9——数字取自重测，不相加也不
// 取一侧。单票沿革：#545 降 1＝统一卖春积极性.ERB 随真身落地（角色編號交換.ERB
// 原为部分移植、落真身后转已移植，不进本计数）；#544（S3）降 1＝魔改新增/
// 强制肉偿.ERB 落 ere/kojo/kojo-forced-payment.js、清单行转「已实现」。
export const PENDING_BASELINE = 8;

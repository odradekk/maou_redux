 ...
 */
export const PENDING_BASELINE = 3; // #542 合并态实测（S0 #541 之后、本票直接落在 25c8365 上）：RULINGS 表加
// DEBUG小白娘2024ver0.0.14.ERB、MOD/ 五个文件、魔改新增/img.ERB 七条判死
// （依据 #540 范围决定 2–4），七份文件从「待移植」直接进「已判定不实现」：
// 10 − 7 = 3，与 `node tools/trace-check.mjs --coverage` 的重测一致（显式
// 改小，非顺手改数字）。剩下的 3 是 S2/#543（處刑改寫.ERB）、S3（强制肉偿.ERB）、
// S4（统一卖春积极性.ERB）的移植目标，各票交付时各自 −1。
// 历次说明保留在下面，不改写。
// #541 合并态实测 10：本票未动任何证据面，S0 只是状态词归一与登记订正，
// 待移植 10 不动（以下为 #541 之前的记录）。
// #467 合并态实测（并上含 #466/#487 的 master cf3b5d9）：先把现役值换成
// 999 占位再跑 `node tools/trace-check.mjs --coverage`，实测 10；master 侧
// 现役 15（#466 的 23 → 15）− 本票的 5（ABL/ABLUP37.ERB／ABLUP39.ERB／

 ...
 */
export const PENDING_BASELINE = 0; // #543+#545 合并态实测（含 #542/#544，master 01d38cd）：
// 已移植 311／部分移植 8／已判定不实现 17／待移植 0／纯声明 10——三票的目标文件
// （#544 强制肉偿、#545 统一卖春积极性、#543 處刑改寫）都已落 ere/，阶段 6 的
// 待移植清空（#540 终点判据第一条）。数字取合并态重测，既不相加也不取一侧；
// 两票各自的沿革：
//   #543（S2）侧：10（#541）− 7（#542 判死 DEBUG小白娘／MOD 五个文件／img.ERB）
//     − 1（#544 强制肉偿）− 1（本票处刑改写）= 1；
//   #545（S4）侧：并上 #542/#544 的合并态实测 1（只剩 S2 的處刑改寫.ERB），
//     再把 统一卖春积极性.ERB 移出待移植（2 → 1），角色編號交換.ERB 落真身后
//     转已移植（后者原为部分移植，不进本计数）。
// 两票各自记的沿革：
//   #543（S2）侧：10（#541）− 7（#542 判死 DEBUG小白娘／MOD 五个文件／img.ERB）
//     − 1（#544 强制肉偿）− 1（本票处刑改写）= 1；
...
//     再把 统一卖春积极性.ERB 移出待移植（2 → 1），角色編號交換.ERB 落真身后
//     转已移植（后者原为部分移植，不进本计数）。
// 两票都落地后应为 0——数字取合并态重测，既不相加也不取一侧；历次说明保留在下面。igin/master
...
/**
 * 收集 `try_kojo(族, '名字'` 的第二实参名（#565 返工第 4 条；函数名随
 * #585 改短，or_stub 后缀已名不副实）：该通道未命中时静默（原作
 * TRYCALLFORM 落空），不打占位——名字不进 `stub_line` 收集面就会彻底失联。
 * 实参名在调用点第二位（kojo-system.js / kojo-dungeon-after.js 的既有写法）。
 * @param {string} text 源文本
 * @returns {string[]}
 */
 ...
 *     实现行）以未了结/终态的那行为准。
 *   - `via: 'try_kojo'`：`try_kojo` 的名字——未命中静默、不占位，
...
export function check_stub_names(entries, registry_text) {
  ...
  for (const { file, name, via = 'stub_line' } of entries) {
    ...
    if (statuses === undefined) {
      failures.push(
        via === 'try_kojo'
          ? `✗ 口上核对锚失联：${file} 的 try_kojo 名字「${name}」在 docs/stub-registry.md 四张表里找不到对应行——该通道未命中即静默，没有清单行就没有任何机械核对`
...
export async function run_coverage({ repo, only = [], list = false }) {
  ...
  // try_kojo 的名字单走「找到清单行即可」的通道（未命中静默、

...
const LEDGER_COUNT_BASELINE = 3296; // #242 +5（M4736-M4740，AEGI 段数、词库与连接符分档）；#248 +9（M6200-M6209，EX 分发路径与 K902 失声）；#249 +40（M6400-M6439，K903 嘉德）；合并后整表加载实测 3128。+1（M3705，test_name 逃生口）；trace-check 提速 +3（M3706-M3708，--only 范围开关）；#321 +1（M3709，串行档的中断可达）；#250 +7（M6500-M6506，K904 菲娅 EX 注册路径）；#331 +13（M6507-M6519，移植状态表 --coverage 的规则与失败判据）；#331 验收整改 +2（M6520-M6521，清单行两形态与归因不到基线）；#332 +23（M6522-M6544，角色侧八个前置函数与接线）；#333 +40（M6562-M6601，段 0 九函数与调用点）；#336 +38（M6642-M6679，录像出售、书架、指令元数据与事件接线）；#335 +40（M6602-M6641，出售估价与零散结算）

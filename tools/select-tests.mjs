...
// 内环与交付闸不必每次跑全量的 143 个文件 / 3890 个用例（实测 139s）。
//
...
// 文件、个位数秒，而全量 139s。这个工具是为内环写的，T2 的那点加速是搭头。
//
...
// 全局锁那 19 个文件（含两份对拍）是纯等待。交付闸（T2）与验收（T3）**不许去锁**。
...
const LOCKS = [
  ...
  'conflict-marker-check', // 已提交的冲突标记：prettier 洗净的 markdown 形态也要红（#299）
];

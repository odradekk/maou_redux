...
脚手架能启动、能跑测试，**游戏本身还没有任何内容**（`ere/main.js` 只有一句 `era.print('Hello World!')`）。在建的是曳光弹「标题画面到主菜单」（issue #15），它把十二个决议串成一条端到端可跑的最短路径。

**移植决议的索引是地图 issue #1**，细节在各自的 ticket 里。地图只读——决议被推翻时回对应 ticket 补勘误，不改写历史。

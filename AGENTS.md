...
| PR 与 master push | CI 全库测试（Linux、Windows 均带引擎）＋ 九份输出比对样本；master 另跑无引擎全库                       |
...
**引擎验收要走到后期流程时，用 `tools/make-acceptance-save.mjs` 造档。** 从新档实打到「人间界已征服」这类前置要上百个游戏日的交互，不现实。做法是两步：先在引擎里开新档、存一个槽（存档结构由引擎自己写），再用该脚本把前置所需的那几个 flag 注入到另一个槽（预设 `campaign` ＝人间界已征服走战役，#471；`ending1` ＝只注满侵攻度走 ENDING_1，#549）。**注入哪些键写在脚本的 `PRESET_PATCHES` 预设表里，改它要同步改对应工单的验收记录**——验收记录必须写明「基础存档是引擎产出的，构造的只有所选预设列出的键」，不能当成「从新档打通」（#471 的用法见该票评论）。
...
流程文档分两份：主 agent 分配任务、验收与合并按 `docs/agents/dispatch-sop.md`（Paseo 用法、并发上限、profile 选用、提示词开场模板、验收清单与阶段验收）；执行工单的 worker 按 `docs/agents/worker-sop.md`（先写测试、用 /code-review 做审查、自己开 PR 等 CI、在 issue 下写完成评论）。

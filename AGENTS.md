...
**曳光弹「标题画面到主菜单」已跑通**（issue #15）：玩家能从标题走到据点主菜单并在其上持续操作，十二个决议被串成一条端到端可跑的最短路径验证了一遍。十七个子系统尚未动工，主菜单的各入口是带记录的存根。
...
- **导入分组排序**：`era` 置顶，其后 `system` / `page` / `event` / `chara` / `kojo` / `utils` / `data` / `i18n`（`chara` = 角色域代码，如 `#/chara/chara-ex`，T6 引入；`kojo` = 口上模块，独立顶层目录，#46 起存在）。

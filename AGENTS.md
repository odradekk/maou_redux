...
- **注释只写「为什么这样写」。** 不写来源标注（外部文件路径、行号），不写「同某处一致」这类脱离当前代码的说明；提到函数名时用当前代码的函数名，解释性注释用当前代码自身能说明的说法，例如「游戏提供 99 个手动存档槽」。口上里的日文分支注释（`それ以外`、`初めて` 等）是分支标记，保留。仓库里既有的来源标注由 #644 统一清理，不要手工删。#11 的 `源:` 文件头约定已废止（#639）。

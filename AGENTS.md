...
**但别在红绿切片的内环里跑全量**（#256）：全量是 94 个文件 / 1386 个用例 / **98 秒**，一个内环跑几十次就是几十分钟的纯等待。分层跑：

| 何时               | 跑什么                                             |
| ------------------ | -------------------------------------------------- |
| **内环**，每个切片 | `npm run test:inner` —— 只跑改动文件相关的测试     |
| **交付前**，一次   | `npm run test:related` + eslint + prettier         |
| **验收 / 阶段闸**  | 全量三项自检 + 全量变异，**由派单人跑**，见 SOP §5 |

`tools/select-tests.mjs` 由改动文件推出测试面，映射取自 `tools/mutations/*.mjs` 的条目表（每条的 `file:`→`tests:` 正是变异检查每次全量都在实证的东西）。`npm run test:select` 只看选中哪些、不跑。

...
test('首回合比对：未解释差异为零，分类计数与当前待办清单一致', async () => {
  ...
  // 【#220（J10）共享 incest() 落地后重测】：@INCEST 在无亲族样本里按原作
  // 静默早退，此前每回合多打的一行存根占位消失，存根 74 → 73。
  // 【#274 回放补装奉仕/SM/重度/特殊后重测】：COM_ABLE30-38/40-49/50-59/80-90
  ...
  // 【#227（J17）触手与自由调教族落地后重测】COM_ABLE100-109/150/208 的真实
  // guard 再滤掉误列菜单，存根 34 → 23。匹配与未解释数不变。

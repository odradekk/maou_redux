 ...
 * 分发表按编号找实现，找不到就落到该族声明的缺席语义上：`missing: 'stub'`
 * 的族打占位行（玩家看见占位文字），`missing: 'silent'` 的族干脆不响
 * （TRYCALL 落空的等价物）。两种都是玩家可见的错。
 *
 ...
 *      分发的权威目录，表与原件对不对得上由 test/event-k-dispatch.test.js 守；
 *   2. 有 `<family>.call(` 调用点的族——`gohoubi_after_koujo_family` 与
 *      `osioski_koujo_family` 住在 kojo-dungeon-after.js，不进分发表，走直接
 *      调用。
 ...
 * 两种来源都推不出前缀时抛：扫描器读不懂新写法必须当场报，不许静默退化成
 * 空集还全绿（与 test/helpers/register-scan.js 同款约定）。
 *
 * == 两道核对 ==
 *   1. 静态：源码里的「定义」与「注册」对照；
 *   2. 运行期：装载主启动图后，每个本体都必须能从**该族的实例**上 `has` 到。
...
/** 全库源码一份，供 `.call(` 扫描 */
const ALL_SOURCE = [...STRIPPED.values()].join('\n');
...
function bodies() {
  const out = [];
  for (const [family, prefixes] of COVERAGE) {
...
/** 契约违反：有本体、该文件却没注册该号。 */
const GAPS = BODIES.filter(({ file, family, id }) => {
...
test('扫描器自检：覆盖族、前缀、本体三样都不为空', () => {
  assert.ok(
    COVERAGE.size >= 18,
    `覆盖族只有 ${COVERAGE.size} 个：${[...COVERAGE.keys()].sort().join(', ')}`,
  );
  ...
  for (const family of [
    ...
    assert.ok(COVERAGE.has(family), `${family} 应在覆盖范围内`);
  }
  // 没有调用路径的两族落在范围外（理由见文件头）
  for (const family of ['dog_kojo_family', 'colosseum_kojo_family']) {
    assert.ok(!COVERAGE.has(family), `${family} 没有调用路径，不应进覆盖范围`);
  }
  // 本体总数是防漂阈值：扫描器读空（正则失配、前缀推断失效）时这里先红，
  // 而不是让下面两条契约核对在空集上「通过」
  assert.ok(
    BODIES.length >= 300,
    `扫到的本体只有 ${BODIES.length} 条，扫描器可能读空了`,
  );

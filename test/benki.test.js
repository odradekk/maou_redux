...
//
// 四个分派的様子句在原作里都是 PRINTFORML（:768/:771/:774/:776、:925/:928/
// :933/:938/:940、:1083/:1086/:1089/:1092/:1094、:1238-1260）——它收尾的是
// 前面名字句/穴句的 PRINTFORM 拼行，整条显示行只换行一次。ere 曾让様子另起
// 一条 print（多一行）。断言用**整行文本**（前缀分支 × 様子分支的表驱动
// 交叉）：片段包含对「拆成两行」是假绿，整行相等才拦得住（#615 同款教训）。

/** 断言某条 text 行整行等于 text（失败时只列尾段相同的候选行） */
function assert_whole_line(lines, text, label) {
...
test('#620 兽奸分派：名字句 + 穴句 + 様子同属一条显示行（:741-763 + :765-776）', async () => {
  ...
  // 様子 :766-776 的四条分支；每行自带分派触发（menu:4 >= 3）——「默认」行
  // 用 :236 的「兽奸经验 > 50」抬高 menu:4，不碰様子自己的三条条件
  const yousu_list = [
    {
      label: '崩坏',
      seed: (f) => {
...
test('#620 奉仕分派：穴句 + 様子同属一条显示行（:892-920 + :922-941）', async () => {
  // 变异条目的 must_mention 出处（断言消息前缀，失败输出里逐字出现）
  ...
  // 様子 :923-941 的五条分支；FLAG:63（常識改変）行用 t151「绝不侍奉」把
  // menu:0 打回 0，避免 menu:0 被 t283 抬到 3 后改走兽奸/奉仕以外的分派
  const yousu_list = [
    {
      label: '常識改変',
    ...
    {
      // :931/:936 的 CALL BENKI_PLAYER_NAME 也落在这一行里（FLAG:64 = 0）
      label: '侍奉快乐>100',
  ...
  // 反感污臭（:914-918）夹在 :912 的「将底层种族」与 :920 的「阴茎温柔地
  // 包裹在内，」之间——仍属同一条显示行
  {
    const { fixture, mod } = setup_benki((f) => {
    ...
    assert_whole_line(
      fixture.text_lines(),
      '用嘴和手将底层种族满是污垢的肮脏的阴茎温柔地包裹在内，看起来很不自在。',
      `${SPOT}（反感污臭）`,
    );
  }
});
...
test('#620 同性爱分派：名字句 + 条件句 + 様子同属一条显示行（:1054-1078 + :1080-1094）', async () => {
  ...
  // 様子 :1081-1094 的五条分支；常識改変行用 t151 把 menu:0 打回 0（t283
  // 会让 menu:0 += 3 → 奉仕分派抢在同性爱之前）
  const yousu_list = [
    {
      label: '常識改変',
      seed: (f) => {
...
test('#620 通常分派：名字句 + 様子同属一条显示行（:1224-1232 + :1236-1260）', async () => {
  // 变异条目的 must_mention 出处（断言消息前缀，失败输出里逐字出现）
  ...
  // 様子 :1236-1260 的十分支；常識改変行同同性爱（t151 压住 menu:0）
  const yousu_list = [
    {
      label: '常識改変',
      seed: (f) => {

 ...
 *   - 断言区间 [85, 115]：种子固定下实测恒 100；±15 的余量给后续票（如
 *     #116 的 @ENDCHECK 真身接入会改变 PRNG 的消费序列，衰减路径随之
 *     漂移），同时两端都远在变异的实测天数之外（衰减归零约 54 日、
 *     /25 改 /30 约 158 日），拦得住工单点名的三类公式错误。
 *
...
test('端到端：新档从标题走到 ENDING_1（三信号 + 实测天数）', async () => {
  ...
  try {
    ...
    // —— @EVENTFIRST：初期奴隶选「村娘」[1]（#50 真身），搬运选「抱起」
    // [1]；村娘分支自己的出口 BEGIN SHOP（链上暂存，emit 返回）——

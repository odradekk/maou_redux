 ...
 *   - 断言区间 [50, 75]：种子固定下实测恒 65。**#502 起由 61 变为 65**——
 *     那一票把 SENGEN_VIDEO_DE 落成真身，日循环每天都多掷一枚 RAND:3，
 *     种子化序列整体移位（61 的来历见下）。区间两端都远在变异的实测天数
 *     之外（衰减归零 40 日、/25 改 /30 直接撞驱动上限），拦得住工单点名的
 *     三类公式错误。61 是 #179 起的值：魔王升级结算真身接入后——侵略给
 *     经验（INVASION.ERB:736-738 的 ELSE 臂）、@LVUP 每回合消化经验、而
 *     魔王补正就是等级本身（INVASION.ERB:568 SINKOU × (CFLAG:0:9 + 100)/100），
 *     存根期被掐掉的「侵略 → 经验 → 升级 → 更强侵略」正反馈第一次跑通，
 *     通关从 100 日缩短到 61 日。
 *
...
test('端到端：新档从标题走到 ENDING_1（三信号 + 实测天数）', async () => {
  ...
  try {
    ...
    // —— 侵攻度与档位的终态 ——
    assert.ok(
      fixture.store.get('flag:81') >= 9901 &&
        fixture.store.get('flag:81') <= 10000,
      '人间界侵攻度曾封顶，终局日衰减后仍在 9901–10000 稳态区间',
    );

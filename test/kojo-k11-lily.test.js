...
// —— SELECTCOM 68（双人侍奉口交 CFLAG:369）——

test('COM68 初めて：玛奥助手淫乱、爱慕与其余三档', async () => {
  for (const talent of [76, 85, undefined]) {
    const fixture = setup_lily((f, era_flag) => {
      preset_chara_17(f);
      f.era.addCharacter(MAO);
    ...
    await speak_com11(fixture, seq_rand());
    assert.ok(fixture.text_lines().length > 0, 'COM68 首次各档均有输出');
    assert.equal(
      fixture.store.get(`cflag:${LILY}:369`),
      1,
      'COM68 首次推进 CFLAG:369=1',
    );
  }
});

test('COM68 初めて：没有玛奥助手时四档保留空模板', async () => {
  for (const item of [{ talent: 76 }, { talent: 85 }, { abl: 3 }, {}]) {
    ...
    await speak_com11(fixture, seq_rand());
    assert.deepEqual(fixture.text_lines(), ['']);
    assert.equal(fixture.store.get(`cflag:${LILY}:369`), 1);
  }
});

test('COM68 二回目：玛奥助手四档与姐妹素质、RAND 分支', async () => {
  for (const target_talent of [76, 85]) {
    const expected = target_talent === 76 ? 5 : 4;
    for (const assi_talent of [85, 76, undefined]) {
      for (const draw of [0, 1, 2]) {
        const fixture = setup_lily((f, era_flag) => {
          preset_chara_17(f);
          f.era.addCharacter(MAO);
          era_flag.assi = MAO;
          era_flag.assiplay = 1;
          f.store.set(`cflag:${LILY}:369`, 1);
          f.store.set(`talent:${LILY}:${target_talent}`, 1);
          if (assi_talent !== undefined)
            f.store.set(`talent:${MAO}:${assi_talent}`, 1);
        }, 68);
        await speak_com11(fixture, seq_rand(draw));
        if (assi_talent === undefined)
          assert.deepEqual(fixture.text_lines(), []);
        else assert.ok(fixture.text_lines().length > 0);
        assert.equal(fixture.store.get(`cflag:${LILY}:369`), expected);
      }
    }
  }

  for (const item of [{ abl: 3, expected: 3 }, { expected: 2 }]) {
    for (const draw of [0, 1, 2]) {
      const fixture = setup_lily((f, era_flag) => {
        preset_chara_17(f);
        f.era.addCharacter(MAO);
      ...
      assert.equal(fixture.store.get(`cflag:${LILY}:369`), item.expected);
    }
  }
});

test('COM68 二回目：没有玛奥助手时四档只推进空模板', async () => {
  const cases = [
    { talent: 76, expected: 5 },
    { talent: 85, expected: 4 },
    { abl: 3, expected: 3 },
...
// —— SELECTCOM 123（乳夹口交 CFLAG:360）——

test('COM123 初めて：淫乱、爱慕、侍奉精神与其余四档', async () => {
  for (const item of [{ talent: 76 }, { talent: 85 }, { abl: 3 }, {}]) {
    const fixture = setup_lily((f) => {
      if (item.talent !== undefined)
        f.store.set(`talent:${LILY}:${item.talent}`, 1);
    ...
    assert.ok(fixture.text_lines().length > 0);
    assert.equal(
      fixture.store.get(`cflag:${LILY}:360`),
      1,
      'COM123 首次推进 CFLAG:360=1',
    );
  }
});

for (const assistant of [true, false]) {
  test(`COM123 二回目：${assistant ? '助手玛奥' : '非助手玛奥'}四档推进`, async () => {
    const cases = [
      { talent: 76, expected: 5 },
      { talent: 85, expected: 4 },
      { abl: 3, expected: 3 },
    ...
    for (const item of cases) {
      ...
      await speak_com11(fixture, seq_rand());
      assert.ok(fixture.text_lines().length > 0);
      assert.equal(fixture.store.get(`cflag:${LILY}:360`), item.expected);
    }
  });
}
...
// —— SELECTCOM 126（手搓口交 CFLAG:362）——

test('COM126 初めて：淫乱、爱慕、侍奉精神与其余四档', async () => {
  for (const item of [{ talent: 76 }, { talent: 85 }, { abl: 3 }, {}]) {
    const fixture = setup_lily((f) => {
      if (item.talent !== undefined)
        f.store.set(`talent:${LILY}:${item.talent}`, 1);
    ...
    assert.ok(fixture.text_lines().length > 0);
    assert.equal(
      fixture.store.get(`cflag:${LILY}:362`),
      1,
      'COM126 首次推进 CFLAG:362=1',
    );
  }
});

for (const assistant of [true, false]) {
  test(`COM126 二回目：${assistant ? '助手玛奥' : '非助手玛奥'}四档推进`, async () => {
    const cases = [
      { talent: 76, expected: 5 },
      { talent: 85, expected: 4 },
      { abl: 3, expected: 3 },
    ...
    for (const item of cases) {
      ...
      await speak_com11(fixture, seq_rand());
      assert.ok(fixture.text_lines().length > 0);
      assert.equal(fixture.store.get(`cflag:${LILY}:362`), item.expected);
    }
  });
}

// —— SELECTCOM 127（真空口交 CFLAG:363）——

test('COM127 初めて：淫乱、爱慕、侍奉精神与其余四档', async () => {
  for (const item of [{ talent: 76 }, { talent: 85 }, { abl: 3 }, {}]) {
    const fixture = setup_lily((f) => {
      if (item.talent !== undefined)
        f.store.set(`talent:${LILY}:${item.talent}`, 1);
    ...
    assert.equal(
      ...
      1,
      'COM127 首次推进 CFLAG:363=1',
    );
  }
});

for (const assistant of [true, false]) {
  test(`COM127 二回目：${assistant ? '助手玛奥' : '非助手玛奥'}四档推进`, async () => {
    const cases = [
      { talent: 76, expected: 5 },
      { talent: 85, expected: 4 },
      { abl: 3, expected: 3 },
    ...
    for (const item of cases) {
      ...
      await speak_com11(fixture, seq_rand());
      assert.ok(fixture.text_lines().length > 0);
      assert.equal(fixture.store.get(`cflag:${LILY}:363`), item.expected);
    }
  });
}
...
// —— SELECTCOM 124（深喉 CFLAG:365；后续门槛误读 CFLAG:363）——

test('COM124 初めて：淫乱、爱慕、侍奉精神与其余四档', async () => {
  for (const item of [{ talent: 76 }, { talent: 85 }, { abl: 3 }, {}]) {
    const fixture = setup_lily((f) => {
      if (item.talent !== undefined)
        f.store.set(`talent:${LILY}:${item.talent}`, 1);
    ...
    await speak_com11(fixture, seq_rand());
    assert.ok(fixture.text_lines().length > 0);
    assert.equal(fixture.store.get(`cflag:${LILY}:365`), 1);
  }
});

for (const assistant of [true, false]) {
  test(`COM124 二回目：${assistant ? '助手玛奥' : '非助手玛奥'}四档推进`, async () => {
    const cases = [
      { talent: 76, expected: 5 },
      { talent: 85, expected: 4 },
      { abl: 3, expected: 3 },
    ...
    for (const item of cases) {
      ...
      await speak_com11(fixture, seq_rand());
      assert.ok(fixture.text_lines().length > 0);
      assert.equal(fixture.store.get(`cflag:${LILY}:365`), item.expected);
    }
  });
}
  ...
  const cases = [
    {
      masturbation: 3,
      expected:
        '莉莉介绍了自己的名字和迄今为止的性经验、自慰的时候幻想的内容和对象也说出来了说得自己都兴奋起来了……',
    },
    {
      masturbation: 0,
      expected: '莉莉介绍了自己的名字和迄今为止的性经验说得自己都兴奋起来了……',
    },
  ];
  for (const item of cases) {
    const fixture = setup_lily((f) => {
      f.store.set(`tequip:${LILY}:53`, 1);
      f.store.set(`talent:${LILY}:89`, 1);
      f.store.set(`abl:${LILY}:31`, item.masturbation);
    }, 56);
    await speak_com11(fixture, seq_rand());
    assert.equal(
      line_with(fixture, '介绍了自己的名字和迄今为止的性经验'),
      item.expected,
      `自慰中毒 ${item.masturbation}`,
    );
  }
});
...
// :6873+:6875+:6877+:6879（初回助手）/ :6901+:6903+:6905+:6907（初回非助手）/
// :6960+:6962+:6964+:6966（二回目助手）/ :6988+:6990+:6992+:6994（二回目非助手）：
  ...
  test(`#623 COM56 ${label}：语言调戏插入段并入整行（三档）`, async () => {
    ...
    for (const [phase, replay, lively, tail] of [
      [
        '初回',
        false,
        assistant ? '却乐在其中' : '乐在其中',
        assistant ? '地回应着。' : '地努力回答着',
      ],
      [
        '二回目',
        true,
      ...
      const cases = [
        { tequip: 11, expected: lively },
        { tequip: 44, expected: '无比痛苦' },
        { tequip: undefined, expected: '' },
      ];
      for (const item of cases) {
        const fixture = setup_lily((f, era_flag) => {
...
test('#623 NTR P=1：狂王插入处女一段整行合并（:12609+:12611+:12613+:12615）', async () => {
  const cases = [
    {
      ...
      flag500: 2,
      expected:
        '双手被抓住的莉莉拼命挣扎着，但狂王只是哈哈大笑着用双腿之间的巨根径直插入了莉莉的处女蜜穴之中。',
    },
    {
      seed: (f) => f.store.set(`talent:${LILY}:76`, 1),
      flag500: 1,
      expected:
        '双手被抓住的莉莉拼命挣扎着，但狂王只是哈哈大笑着用粗大的假阳具径直插入了莉莉的处女蜜穴之中。',
    },
    {
      seed: () => {},
      flag500: 2,
  ...
  for (const item of cases) {
    ...
    const { ntr_koujo_family } = fixture.load_module('kojo/kojo-system');
    await ntr_koujo_family.call(11, { args: [seq_rand(), 1] });
    assert.equal(
      line_with(fixture, '拼命挣扎着，但狂王只是哈哈大笑着用'),
      item.expected,
      `FLAG:500=${item.flag500}`,
    );
  }
});
...
test('#623 NTR P=2：肛门插入两段整行合并（:12638+… / :12655+…）', async () => {
  ...
  for (const item of cases) {
    ...
    const { ntr_koujo_family } = fixture.load_module('kojo/kojo-system');
    await ntr_koujo_family.call(11, { args: [seq_rand(), 2] });
    assert.equal(
      item.expected.includes('的肛门被')

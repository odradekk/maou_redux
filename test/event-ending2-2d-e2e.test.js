...
test('端到端：2D 模式新档从标题走到 ENDING_2（LABO_DUNGEON_MAP:175 的 JUMP 可达）', async () => {
  ...
  try {
    ...
    // 此后缓慢回吐侵攻度），断言收敛在「接近封顶」区间，与 FLAG:82 / 横幅
    // 两证互补。
    //
    // **这个下界是重测值，不是设计值。** Math.random 被整场替换成种子 PRNG
    // （:149），所以任何一张票新增随机取数都会平移此后的整条序列、剧本随之
    // 走上别的分支。已经动过两次：#342 接通结婚日（未婚角色的恋人判定入列，
    // 终局约 8775）；#385 让角色年龄与身体数据落真身后，全场取数由 27,573
    // 涨到 45,716（chara-body 自身只被调用 18 次，增量全在下游——角色有了
    // 真实年龄之后走的是另一条剧本），终局实测 7931。
    //
    // 下界因此留足余量：它要拦的是「侵略线整个没推进」那一类塌陷，而那一类
    // 本就先被上面的 FLAG:82 拦住。序列平移属正常，不该在这里报红。
    const invasion = fixture.store.get('flag:81') ?? 0;
    assert.ok(
      invasion > 7000,
      `FLAG:81 接近封顶（ENDING_1 的触发条件留证，衰减回吐后仍 > 7000；实测 ${invasion}）`,
    );
    assert(

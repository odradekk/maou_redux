...
test('口上域切片：cflag 属主 kojo 的下标恰好是命名表的 110 条', () => {
  ...
  // #234（J24 K3）补 2 个：7（train 穿孔装着——COM87/口上读 CFLAG:7 位图）、
  // 504（stronghold 要求奖赏——GOHOUBI_REQUEST 读 CFLAG:504）
  const NON_KOJO_NAMED = [
    ...
    534, 550, 551, 552, 570, 580, 582, 601, 666, 667,
  ];
...
test('尾部条目分区（named_tail，#170）：同表既有条目之后发射、两侧各自升序', () => {
  ...
  const expected_tails = {
    ...
    invasion: [521],
    patch: [120, 582],
    train: [5, 15, 16, 32, 40, 41, 45, 46, 61, 81, 82, 491, 499, 666],
    system: [101, 103, 104],
  };

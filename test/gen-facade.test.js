...
test('口上域切片：cflag 属主 kojo 的下标恰好是命名表的 110 条', () => {
  ...
  // 602（chara 结婚爱情）
  // #245（J35 K14）补 1 个：70（stronghold 已性转，K14 的性転換済み分支读它）
  // #336 补 493/495/496/498（录像价值、浏览、粉丝信与属性）；#339 补 0
  //（出售与助手资格）；#346 补 112/113；#350 补 190（通信勇者唯一标记）；
  // #391 补 700（chara 收藏——角色信息页收藏切换跨域写）、777（patch 待
  // 处刑标签——拘束台解放跨域清位）。
  ...
  const NON_KOJO_NAMED = [
    ...
    43, 44, 45, 46, 47, 48, 49, 50, 61, 70, 81, 82, 101, 102, 103, 104, 105,
    106, 107, 108, 109, 110, 111, 112, 113, 120, 130, 131, 151, 190, 451, 452,
    491, 493, 495, 496, 498, 499, 500, 501, 502, 503, 504, 505, 506, 507, 508,
    510, 511, 521, 534, 550, 551, 552, 570, 580, 582, 601, 602, 606, 607, 608,
    610, 666, 667, 700, 777, 820, 999,
  ];
...
test('尾部条目分区（named_tail，#170）：同表既有条目之后发射、两侧各自升序', () => {
  ...
  const expected_tails = {
    ...
    chara: [6, 42, 151, 601, 602, 700, 820],
    dungeon: [
      11, 12, 105, 106, 107, 112, 501, 505, 508, 580, 606, 607, 608, 610,
    ],
    ...
    // 只是没人核过——49 是本票新增）
    stronghold: [44, 49, 70, 493, 495, 496, 498, 500, 504],
    train: [
      3, 5, 7, 15, 16, 32, 40, 41, 43, 45, 46, 47, 48, 61, 81, 82, 113, 491,
      499, 666,
    ],
    system: [101, 103, 104, 108],
  };

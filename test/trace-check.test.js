 ...
 *      塌成 0，判断条件就落在这个 0 上（全量跑法上扫描面塌陷会被 #513 的过期
 *      规则先拦下，计数核对执行不到）。
 *
...
/**
 * #538：把「完整性扫描覆盖 ere/kojo/」这条正面锁住。
 *
 * 上面那条全绿用例把两个计数都交给**全量**跑法，而扫描面整块塌掉时工具会
 * 先被别的规则拦下（M9301 的实测现场：`ere/kojo/kojo-k10-club.js` 的
 * SRC_MISBIND_BASELINE 条目扫不到，报「#513 基线条目已不再报错绑」），
 * `assert.equal(status, 0)` 当场就红，计数核对根本没执行到——于是那条
 * 用例证明的是「工具红了」，不是「扫描面塌了」。
 *
 * 所以这里换一个**取值范围**：`--only` 收窄到一个没有 SRC_MISBIND_BASELINE
 * 条目的口上文件，扫描面塌掉时工具仍全绿（没有基线条目可过期），而报告里的
 * 完整性计数会塌成 0。断言就落在这个 0 上。
 *
 * 不复用 ref_counts_consistent 的 1% 量级判断条件：那个余量是**全量**统计
 * 方式下两侧差额（净差 +47）量出来的，单个文件的重复登记会把差额放大到 2%
 * 以上（实测 kojo-k0-tender：锚表 5100 / 扫出去重 4996），判断条件在这里恒假。
 *
 * 选 kojo-k10-club.js 之外的文件是有意的：它在 SRC_MISBIND_BASELINE 里，
 * 收窄到它会被 #513 的过期规则先拦下（那正是 M9301 在全量跑法上的路径）。
 */
test('#431 扫描面：单独跑一个口上文件时完整性计数不得为 0（ere/kojo/ 必须在面内）', () => {
  const one = 'ere/kojo/kojo-k0-tender.js';
  assert.ok(
    !fs.readFileSync(TOOL, 'utf8').includes(`'${one}': [`),
    `${one} 不该有 SRC_MISBIND_BASELINE 条目——有的话这条用例会被 #513 的过期规则先拦下`,
  );
  const { status, output } = run_tool(['--only', one]);
  assert.equal(status, 0, `--only ${one} 应全绿：\n${output}`);
  const m = output.match(
    /✓ \d+ 条内联行号引用全部与源文件一致（本次限定范围：--only ere\/kojo\/kojo-k0-tender\.js[^）]*）；ERB 完整性：ere\/ (\d+) 条引用/,
  );
  assert.ok(m, `trace-check 输出形状变了：\n${output}`);
  assert.ok(
    Number(m[1]) > 0,
    'ere/kojo/ 不再进完整性扫描：单独跑一个口上文件时扫出的引用数为 0（扫描面塌陷，#431）',
  );
});
...
test('#513 错绑基线：基线外的错绑红；条目不再错绑（已消化）时必须删', () => {
  ...
  try {
    ...
    // 只断言「非零退出」不够（#538/M11064）：塞一条假条目同时把条目总数顶过
    // 冻结上界，于是「超出 #513 冻结上界」自己就能把它染红，`includes('#513')`
    // 又被那条报错文案里的「#513」满足——「过期基线条目」这条规则整条失效时
    // 仍全绿。这里改成报出**规则自己的报错文案**与那条假条目：过期规则不开火
    // 就没这两样（上界报错只说条数与上界，不报条目名）。
    assert.ok(
      r.output.includes('999999') && r.output.includes('已不再报错绑'),
      '必须报出过期基线条目本身（只报条目总数上界不算）——过期规则没有开火：\n' +
        r.output,
    );
...
test('移植状态表（#542）：DEBUG/MOD/立绘七文件判已判定不实现，待移植只剩 S2 的處刑改寫.ERB', () => {
  ...
  // 合并 S3（#544 的强制肉偿.ERB）与 S4（#545 的统一卖春积极性.ERB）后，
  // 阶段 6 收口前剩下的待移植只有 S2 的處刑改寫.ERB——它交付时更新这里
  ...
  assert.deepEqual(
    pending,
    ['target/ERB/魔改新增/處刑改寫.ERB'].sort(),
    `待移植必须只剩 S2 的一个文件（PENDING_BASELINE = 1）：\n${output}`,
  );
});

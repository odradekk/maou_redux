 ...
 * `assert.equal(status, 0)` 当场就红，计数判据根本没执行到——于是那条
 ...
 *
 * 不复用 ref_counts_consistent 的 1% 量级判据：那个余量是**全量**口径下
 * 两侧差额（净差 +47）量出来的，单个文件的重复登记会把差额放大到 2%
 * 以上（实测 kojo-k0-tender：锚表 5100 / 扫出去重 4996），判据在这里恒假。
 *
...
test('#513 错绑基线：基线外的错绑红；条目不再错绑（已消化）时必须删', () => {
  ...
  try {
    ...
    // 仍全绿。这里改成点名**规则自己的报错文案**与那条假条目：过期规则不开火
    // 就没这两样（上界报错只说条数与上界，不点条目名）。
    assert.ok(
      r.output.includes('999999') && r.output.includes('已不再报错绑'),
      '必须点名过期基线条目本身（只报条目总数上界不算）——基线只收真实的存量：\n' +
        r.output,
    );

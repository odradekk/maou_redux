...
/**
 * 满旋钮角色（#591）：在 love_world 的底座上把每个能加分的表都点亮——用来把
 * 「超过显示门槛的喜好」个数喂到显示面的上限。旋钮取自 love_score 里的逐条加分：
 * 素质走 IF TALENT:N、能力 0-3/16/17/20-23/30-39 是无条件入项、经验与刻印各一组。
 * @returns {object} love_world 的同款句柄
 */
function max_love_world() {
  const w = love_world();
  for (const idx of [
    9, 23, 24, 30, 31, 35, 36, 40, 47, 57, 60, 74, 75, 76, 77, 78, 80, 83, 85,
    88, 89, 101, 102, 103, 104, 105, 106, 107, 108, 121, 124, 130, 140, 141,
    142, 143, 150, 151, 157, 180, 181, 204, 230, 231, 232, 233, 273, 280, 317,
  ]) {
    w.set_talent(idx, 1);
  }
  w.set_talent(314, 1); // 种族 = 精灵（高洁）
  w.set_talent(315, 2); // 前职业 = 修道女
  w.set_talent(316, 16); // 契机 = 平和
  w.set_cflag(0, 2); // 沦落度 2
  w.set_cflag(602, 9); // 新的结婚对象 → 41
  w.set_cflag(607, 9); // 恋人 → 42
  w.set_cflag(606, 200); // LOVER = 200（「恋人」措辞支）
  for (const idx of [
    0, 1, 2, 3, 10, 11, 12, 13, 14, 16, 17, 20, 21, 22, 23, 30, 31, 32, 33, 37,
    39,
  ]) {
  ...
  for (const idx of [8, 21, 23, 30, 32, 33, 74, 75]) {
    w.set_exp(idx, 101);
  }
  w.set_mark(1, 3); // 快乐刻印 → 50
  return w;
}

/**
 * 显示面的上限（#591）：满旋钮角色把每个能出文本的喜好都喂过门槛，整屏 22 项、
 * 末尾一项落在 rank 22。
 *
 * **为什么没有「LOVE_SORT_MAX 30 → 29 / 31」的边界用例**：显示循环扫的排名次上界
 * 是 LOVE_SORT_MAX（30），但显示面的名额只有 28 —— 分支表覆盖 0-4/10-13/20-22/
 * 30-35/40-42/51-54/60-62（29 个 id，去掉恒不显示的 50），其中 3/4 的分数涨不过
 * 门槛、再扣 1 个不出文本。能写 MAIN_LOVE 的下标就只有这些，所以「分数 > 3 的
 * 下标」最多 28 个，rank 29/30 上永远是分数 ≤ 3 的下标、必被门槛 continue 掉。
 * 结论：LOVE_SORT_MAX 在 [23, 100] 里取任何值都与 30 逐字同输出（等价变异，实测
 * 见 #591 完成报告：把 25…32 逐个代入排序+显示，22 项与顺序完全一致）。
 *
 * 本用例改钉三件事——显示项数与整屏顺序（22 项，mutation: LOVE_SORT_MAX 压到
 * 22 会少一项）、门槛与心形上限的字面量（3 / 6：末项「丈夫」分值 4、首项心形 6）、
 * 以及名额表本身没长到 30（长到了这条会红，那时才轮到补真边界用例）。
 */
test('look_info_love：满旋钮角色的显示面上限（名额 28 < LOVE_SORT_MAX）', async () => {
  const w = max_love_world();
  await w.mod.look_info_love(w.cid);
  assert.deepEqual(w.fixture.text_lines(), [
    '[喜欢的东西]',
    '　人妻♡♡♡♡♡♡　中年大叔♡♡♡♡♡♡　萝莉的小穴♡♡♡♡♡♡　正太的阴茎♡♡♡♡♡♡　做爱♡♡♡♡♡　精液♡♡♡♡♡　 ',
    '　狂王大人♡♡♡♡♡　百合行为♡♡♡♡　卖淫♡♡♡　被人虐待♡♡♡　露出身体♡♡♡　和野兽交配♡♡♡　 ',
    '　虐待别人♡♡　甜食♡♡　野狗大人♡♡　被弄菊穴♡　被弄乳房♡　被弄阴蒂♡　 ',
    '　被弄小穴　现在的丈夫　恋人　丈夫　 ',
    '[共22个喜欢的东西]',
  ]);
  const over = w.score().filter((v) => v > 3).length;
  assert.equal(
    over,
    23,
    '满旋钮角色里越过显示门槛的下标数（23 个里含恒不显示的 50 → 能出 22 项）',
  );
  const src = fs.readFileSync(
    path.join(__dirname, '..', 'ere', 'chara', 'look.js'),
    'utf8',
  ...
  const branches = new Set(
    [...src.matchAll(/\bid === (\d+)\)/g)].map((m) => Number(m[1])),
  );
  assert.ok(
    branches.size < 30,
    `显示分支的 id 个数 ${branches.size} 已达/超过 LOVE_SORT_MAX（30）：上界开始咬边界，` +
      '必须补「30 项上限」的边界用例与「改成 29 / 31」的变异条目（#591 实测它们现在是等价变异）',
  );
});

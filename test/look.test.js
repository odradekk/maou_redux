...
test('LOOK_SET 素质 316：理由三档的善恶值', () => {
  for (const [roll, expected, delta] of [
    ...
    // （源 :593、:612、:650、:717、:764）——只钉第一处，其余给不增减的值
    const cid = w.run(
      0,
...
test('love_score 经验補正：七组各自的档位（肛门组 >200/>80，其余 >100/>30）', () => {
  ...
  const groups = [
    ...
    [23, 1, 100, 30], // 爱情经验 → 你
    [30, 33, 100, 30], // 被虐快乐经验 → 受虐
    [32, 12, 200, 80], // 肛门快乐经验 → A性感（唯一一组两档都不同）
    [33, 34, 100, 30], // 施虐快乐经验 → 施虐
    [75, 22, 100, 30], // 营业爱情经验 → 卖淫
  ];
  for (const [exp_idx, target, hi, mid] of groups) {
    // 两档的四个边界各钉一次：> hi 得 3、hi 本身掉到 2；> mid 得 2、mid 掉到 1
    for (const [value, delta] of [
      ...
      assert.equal(
        w.score()[target] - base[target],
        delta,
        `EXP:${exp_idx} = ${value} → LOVE:${target}（档位 ${hi}/${mid}）`,
      );
    }
  }
});
